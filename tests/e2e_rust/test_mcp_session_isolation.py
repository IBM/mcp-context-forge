...
def _assert_denied(response: httpx.Response) -> None:
    ...
    if response.status_code in (403, 404):
        if response.headers.get("content-type", "").startswith("application/json"):
            ...
            assert "denied" in body_text or "not found" in body_text or "access" in body_text, body
        return
    ...
    if "error" in body:
        ...
        assert "denied" in str(error.get("message", "")).lower() or "not found" in str(error.get("message", "")).lower(), body
        return
    ...
    assert "denied" in message or "not found" in message, body
