...
TLS+AUTH variant: this copy targets a Redis that requires BOTH in-transit
encryption (rediss://) and an auth token (requirepass) — mirroring AWS
ElastiCache with encryption + an AUTH token. The plugin's connection comes
from plugins/config.yaml (rediss://:<pw>@... + redis_ssl_ca_certs); all the
docker-exec redis-cli helpers below are patched to speak --tls and -a <pw>.

Prerequisites:
    - cpex-rate-limiter installed in the gateway venv (gateway loads the plugin)::
        uv pip install "cpex-rate-limiter==0.1.3"
    - Three standalone docker containers running:
        rl-pg         (postgres:18, port 5432:5432, db=mcp, user/pass=postgres/postgres)
        rl-redis      (redis:latest, TLS-only + requirepass, port 6379:6379)::
          ...
          (mount the repo's tls-certs/ at /certs; password = REDIS_CLI_PASSWORD)
    ...
    - Gateway running locally via ``make dev`` with env::
        ...
        REDIS_URL='rediss://:$REDIS_CLI_PASSWORD@127.0.0.1:6379/0'
        REDIS_SSL=true
        REDIS_SSL_CA_CERTS='<repo>/tls-certs/ca.crt'
        CACHE_TYPE='redis'      # or 'memory' to isolate the plugin's Redis
        AUTH_REQUIRED=false
...
# TLS-only + password-protected Redis variant: the standalone rl-redis in this
# test is started with `--tls-port 6379 --requirepass ...`, so every redis-cli
# invocation must speak TLS and authenticate. `--insecure` skips server-cert
# verification for the CLI helper only (the gateway/plugin still verify via
# redis_ssl_ca_certs). Set REDIS_CLI_PASSWORD to match the requirepass value
# you launched rl-redis with; the default below is a placeholder only.
REDIS_PASSWORD = os.environ.get("REDIS_CLI_PASSWORD", "changeme")  # pragma: allowlist secret
REDIS_CLI_TLS = ["--tls", "--insecure", "--no-auth-warning", "-a", REDIS_PASSWORD]
...
def _redis_rl_keys(team_id: str) -> list[tuple[str, str, str]]:
    ...
    try:
        keys_out = subprocess.run(
            ["docker", "exec", REDIS_CONTAINER, "redis-cli", *REDIS_CLI_TLS,"--scan", "--pattern", pattern],
    ...
    for k in (line.strip() for line in keys_out.splitlines() if line.strip()):
        try:
            v = subprocess.run(
                ["docker", "exec", REDIS_CONTAINER, "redis-cli", *REDIS_CLI_TLS,"GET", k],
                capture_output=True, text=True, timeout=5, check=True,
            ).stdout.strip()
            t = subprocess.run(
                ["docker", "exec", REDIS_CONTAINER, "redis-cli", *REDIS_CLI_TLS,"TTL", k],
                capture_output=True, text=True, timeout=5, check=True,
            ).stdout.strip()
