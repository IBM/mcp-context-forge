...
class ContainerManager:
    ...
    def start_sqlite_container(self, version: str, 
                              ...
                              extra_env: Dict[str, str] = None) -> str:
            ...
            try:
                ...
                try:
                    ...
                    if curl_result.returncode == 0:
                        logger.info(f"✅ Container {container_id[:12]} is ready and healthy")
                        return
    ...
    def get_database_schema(self, container_id: str, db_type: str) -> str:
        ...
        """
        logger.info(f"📋 Extracting {db_type} schema from {container_id[:12]}")
        
        if db_type.lower() == "sqlite":
            cmd = [
                self.runtime, "exec", container_id, 
                "sqlite3", "/app/data/mcp-alembic-migration-test.db", ".schema"
            ]
        else:  # PostgreSQL
            cmd = [
                self.runtime, "exec", container_id,
                "pg_dump", "--schema-only", "--no-owner", "--no-privileges",
                "postgresql://test_user:test_migration_password_123@localhost:5432/mcp_test"
            ]
        
        result = self._run_command(cmd, capture_output=True)
        logger.info(f"✅ Schema extracted ({len(result.stdout)} characters)")
        return result.stdout
