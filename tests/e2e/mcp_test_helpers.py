...
def _rust_mcp_gateway_active() -> bool:
    try:
        ...
        if resp.status_code != 200:
            return False
        return resp.headers.get("x-contextforge-mcp-runtime-mode") == "rust-managed"
    except Exception:
        return False
...
skip_no_rust_mcp_gateway = pytest.mark.skipif(not _rust_mcp_gateway_active(), reason=f"Rust MCP gateway not active at {BASE_URL}")
