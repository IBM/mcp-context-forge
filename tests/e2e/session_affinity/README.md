...
The script prints a per-test `PASS/FAIL` summary and exits non-zero on failure.

## Notes

- These run against `http://localhost:8080` (nginx) by default; override with `GW_BASE`.
- The counter server must bind `0.0.0.0` (it does by default) and be registered via
  `http://host.docker.internal:9400/mcp` — not `localhost` — so the gateway containers can
  reach it. `run_reproducers.py` handles this registration for you.
