...
class TestUpdateApiTokenLastUsed:
    ...
    @pytest.mark.asyncio
    async def test_api_token_last_used_updated_on_jwt_auth(self):
    ...
    @pytest.mark.asyncio
    async def test_api_token_jti_stored_in_request_state(self):
    ...
    @pytest.mark.asyncio
    async def test_legacy_api_token_last_used_updated(self):
                    ...
                    with patch("mcpgateway.auth._is_api_token_jti_sync", return_value=True):
                        with patch("mcpgateway.auth._update_api_token_last_used_sync") as mock_update:
                            with patch("mcpgateway.auth.asyncio.to_thread", AsyncMock(side_effect=lambda f, *args: f(*args))):
                                ...
                                # Called twice: once for _is_api_token_jti_sync check, once for update
