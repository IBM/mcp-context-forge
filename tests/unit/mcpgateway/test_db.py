...
def test_db_module_otel_sqlalchemy_instrumentation_success(monkeypatch, caplog):
    # Standard
    import importlib
    import sys
    import types
    ...
    original_url = db.settings.database_url
    original_otel_obs = db.settings.otel_enable_observability
    original_otel_sql = db.settings.otel_sqlalchemy_instrumentation_enabled
    ...
    dummy_mod = types.ModuleType("opentelemetry.instrumentation.sqlalchemy")
    dummy_mod.SQLAlchemyInstrumentor = FakeSQLAlchemyInstrumentor
    ...
    try:
        ...
        monkeypatch.setitem(sys.modules, "opentelemetry.instrumentation.sqlalchemy", dummy_mod)
        ...
        monkeypatch.setattr(db.settings, "database_url", original_url)
        monkeypatch.setattr(db.settings, "otel_enable_observability", original_otel_obs)
        monkeypatch.setattr(db.settings, "otel_sqlalchemy_instrumentation_enabled", original_otel_sql)
        importlib.reload(db)


def test_db_module_otel_sqlalchemy_instrumentation_importerror(monkeypatch, caplog):
    # Standard
    import builtins
    import importlib
    ...
    try:
        ...
        assert any("opentelemetry-instrumentation-sqlalchemy not installed" in record.message for record in caplog.records)
    finally:
        monkeypatch.setattr(db.settings, "database_url", original_url)
        monkeypatch.setattr(db.settings, "otel_enable_observability", original_otel_obs)
        monkeypatch.setattr(db.settings, "otel_sqlalchemy_instrumentation_enabled", original_otel_sql)
