...
def _trusted_internal_request(path, *, marker="rust", hmac_value=None, ctx="ctx", client="127.0.0.1"):
    """Build a real loopback internal request for the CSRF trust-gate tests."""
    # First-Party
...
@pytest.mark.asyncio
async def test_internal_mcp_dispatch_csrf_exempt_via_trust_gate():
    """The trusted-internal dispatch is CSRF-exempt through the shared trust gate.

    The exemption is contingent on the full gate (loopback + HMAC + runtime marker +
    auth context), not on a CSRF path-prefix entry, so it works for custom
    AUTH_HEADER_NAME deployments while remaining unreachable from an untrusted client.
    """
    middleware = CSRFMiddleware(app=AsyncMock())
    call_next = AsyncMock(return_value=Response("ok", status_code=200))
    ...
    with patch("mcpgateway.middleware.csrf_middleware.settings") as mock_settings:
        ...
        mock_settings.csrf_exempt_paths = []  # no path-prefix exemption; the trust gate is the only skip
...
@pytest.mark.asyncio
async def test_internal_mcp_path_not_csrf_exempt_when_trust_gate_unsatisfied():
    """An external POST to ``/_internal/mcp/rpc`` is NOT CSRF-exempt by path alone.

    With a forged HMAC the trust gate fails, so the request falls through to normal
    CSRF enforcement and is rejected (no token, no bearer).
    """
    middleware = CSRFMiddleware(app=AsyncMock())
    call_next = AsyncMock(return_value=Response("ok", status_code=200))
    request = _trusted_internal_request("/_internal/mcp/rpc", hmac_value="forged-not-the-real-hmac")
