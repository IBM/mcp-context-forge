...
class TestTokenScopingMiddleware:
    ...
    @pytest.mark.asyncio
    async def test_internal_mcp_request_without_runtime_auth_header_does_not_bypass(self, middleware, mock_request):
        """An unsigned auth context without the secret-derived trust header must not bypass token scoping."""
        mock_request.url.path = "/_internal/mcp/rpc"
        mock_request.scope["path"] = "/_internal/mcp/rpc"
        mock_request.method = "POST"
        # Auth-context present but no x-contextforge-mcp-runtime-auth: untrusted.
        mock_request.headers = {
            "Authorization": "Bearer scoped-token",
            "x-contextforge-auth-context": "self-minted-unsigned-payload",
        }
