...
async def test_manager_single_transformer_prompt_plugin():
    ...
    result, _ = await manager.invoke_hook(HookType.PROMPT_POST_FETCH, payload_result, global_context=global_context, local_contexts=contexts)
...
async def test_manager_tool_hooks_empty():
    ...
    result, contexts = await manager.invoke_hook(HookType.TOOL_POST_INVOKE, tool_result_payload, global_context=global_context)
...
async def test_manager_tool_hooks_with_transformer_plugin():
    ...
    result, _ = await manager.invoke_hook(HookType.TOOL_POST_INVOKE, tool_result_payload, global_context=global_context, local_contexts=contexts)
