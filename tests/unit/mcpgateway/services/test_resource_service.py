...
@pytest.mark.asyncio
async def test_admin_team_id_scopes_to_team_and_keeps_public(resource_service, monkeypatch):
    ...
    monkeypatch.setattr("mcpgateway.services.resource_service._get_registry_cache", lambda: mock_cache)

    mock_paginate = AsyncMock(return_value=([], None))
    monkeypatch.setattr("mcpgateway.services.resource_service.unified_paginate", mock_paginate)
    monkeypatch.setattr("mcpgateway.services.base_service.is_user_admin", MagicMock(return_value=True))

    await resource_service.list_resources(
        db,
    ...
    assert "resources.team_id = 'team-1'" in compiled
    # ... but globally-public rows from any team are still ORed in.
    assert "resources.visibility = 'public'" in compiled
