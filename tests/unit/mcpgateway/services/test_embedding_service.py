...
class TestEmbeddingServiceBasic:
    ...
    async def test_initialize_success(self, mock_factory):
        ...
        mock_embedding_model.aembed_query = AsyncMock(return_value=[0.1] * 1536)
    ...
    async def test_embed_query_success(self, mock_factory):
        ...
        mock_embedding_model.aembed_query = AsyncMock(return_value=expected_embedding)
        mock_factory.create.return_value = mock_provider
        mock_provider.get_embedding_model.return_value = mock_embedding_model
        ...
        mock_embedding_model.aembed_query.assert_called_with("weather tools")
...
class TestEmbedTool:
    ...
    async def test_embed_tool_success(self, mock_factory, sample_tool_data):
        ...
        mock_embedding_model.aembed_query.assert_called_with(expected_text)
...
class TestEmbedText:
    ...
    async def test_embed_text_success(self, mock_factory):
        ...
        assert result == expected_embedding
        mock_embedding_model.aembed_query.assert_called_with("Some text to embed")
...
class TestBatchEmbedTools:
    ...
    async def test_batch_embed_tools_success(self, mock_factory, sample_tool_data):
        ...
        mock_embedding_model.aembed_query = AsyncMock(return_value=[0.1] * 1536)
        mock_embedding_model.aembed_documents = AsyncMock(return_value=expected_embeddings)
        mock_factory.create.return_value = mock_provider
        mock_provider.get_embedding_model.return_value = mock_embedding_model
...
class TestErrorHandling:
    ...
    async def test_embed_text_provider_error_raises_runtime_error(self, mock_factory):
        ...
        # First call (probe) returns valid 1536-dim vector; second call (actual) raises
        mock_embedding_model.aembed_query = AsyncMock(
            side_effect=[[0.1] * 1536, Exception("provider failure")]
        )
        mock_factory.create.return_value = mock_provider
        mock_provider.get_embedding_model.return_value = mock_embedding_model
    ...
    async def test_batch_embed_tools_provider_error_raises_runtime_error(self, mock_factory, sample_tool_data):
        ...
        mock_embedding_model.aembed_query = AsyncMock(return_value=[0.1] * 1536)
        mock_embedding_model.aembed_documents = AsyncMock(side_effect=Exception("batch failure"))
        mock_factory.create.return_value = mock_provider
        mock_provider.get_embedding_model.return_value = mock_embedding_model
    ...
    async def test_batch_embed_texts_provider_error_raises_runtime_error(self, mock_factory):
        ...
        mock_embedding_model.aembed_query = AsyncMock(return_value=[0.1] * 1536)
        mock_embedding_model.aembed_documents = AsyncMock(side_effect=Exception("batch text failure"))
        mock_factory.create.return_value = mock_provider
        mock_provider.get_embedding_model.return_value = mock_embedding_model
...
class TestEmbedAndStoreTool:
    ...
    async def test_embed_and_store_tool_success(self, mock_factory, mock_db_session):
        ...
        mock_embedding_model.aembed_query.assert_called()
        mock_db_session.add.assert_called_once()
        mock_db_session.commit.assert_called_once()
...
# ---------------------------------------------------------------------------
# Tests for race-condition fix (asyncio.Lock) and API key exposure fix
# ---------------------------------------------------------------------------


class TestInitializeLock:
    """Verify that concurrent calls to initialize() are safe."""

    @pytest.mark.asyncio
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_concurrent_initialize_calls_provider_once(self, mock_settings, mock_factory):
        """Under concurrent load, the provider is created exactly once."""
        mock_settings.embedding_provider = "openai"
        mock_settings.embedding_model = "text-embedding-3-small"
        mock_settings.embedding_api_key = SecretStr("sk-test")
        ...
        # Provider must have been created exactly once
        mock_factory.create.assert_called_once()
        mock_provider.get_embedding_model.assert_called_once()
        assert service._initialized is True

    @pytest.mark.asyncio
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_second_initialize_call_is_noop(self, mock_settings, mock_factory):
        """Calling initialize() after the first time is a no-op."""
        mock_settings.embedding_provider = "openai"
        mock_settings.embedding_model = "text-embedding-3-small"
        mock_settings.embedding_api_key = SecretStr("sk-test")
        mock_settings.embedding_dim = 1536
    ...
    def test_lock_exists_on_new_instance(self):
        """EmbeddingService has a Lock from the moment it is created."""
        service = EmbeddingService()
        assert isinstance(service._lock, asyncio.Lock)


class TestApiKeyExposure:
    """Verify that the API key stays as SecretStr until the LangChain boundary."""

    @patch("mcpgateway.services.embedding_service.settings")
    def test_api_key_not_accessed_during_init(self, mock_settings):
        """__init__ reads provider/model names but never touches the api_key value."""
        mock_settings.embedding_provider = "openai"
        mock_settings.embedding_model = "text-embedding-3-small"
        mock_settings.embedding_api_key = SecretStr("sk-test")
    ...
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_api_key_stays_as_secretstr_in_initialize(self, mock_settings, mock_factory):
        """initialize() passes SecretStr directly to OpenAIEmbeddingConfig without unwrapping it."""
        mock_settings.embedding_provider = "openai"
        mock_settings.embedding_model = "text-embedding-3-small"
        mock_settings.embedding_api_key = SecretStr("sk-test")
        mock_settings.embedding_dim = 1536
        ...
        # The config passed to the factory must still hold a SecretStr, not a plain string
        call_args = mock_factory.create.call_args[0][0]
        assert isinstance(call_args.config.api_key, SecretStr)
        assert call_args.config.api_key.get_secret_value() == "sk-test"

    @pytest.mark.asyncio
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_provider_created_only_once_on_repeat_initialize(self, mock_settings, mock_factory):
        """Repeat calls to initialize() do not re-create the provider."""
        mock_settings.embedding_provider = "openai"
        mock_settings.embedding_model = "text-embedding-3-small"
        mock_settings.embedding_api_key = SecretStr("sk-test")
        mock_settings.embedding_dim = 1536
        ...
        service = EmbeddingService()
        await service.initialize()
        await service.initialize()  # second call — must not re-create provider

        mock_factory.create.assert_called_once()

    @patch("mcpgateway.services.embedding_service.settings")
    def test_model_name_accessible_before_initialize(self, mock_settings):
        """_model_name and _provider_name are set in __init__ without touching the api_key."""
        mock_settings.embedding_provider = "openai"
        mock_settings.embedding_model = "text-embedding-3-small"
        mock_settings.embedding_api_key = SecretStr("sk-test")
        ...
        assert service._model_name == "text-embedding-3-small"
        assert service._initialized is False


# ---------------------------------------------------------------------------
# Tests for embedding dimension detection and mismatch validation
# ---------------------------------------------------------------------------


class TestEmbeddingDimension:
    """Verify probe-based dimension detection and hard mismatch rejection."""

    def _make_service_and_factory(self, mock_settings, mock_factory, model_name, probe_dim, configured_dim):
        """Helper: wire up mocks and return a fresh EmbeddingService."""
        mock_settings.embedding_provider = "openai"
        mock_settings.embedding_model = model_name
        mock_settings.embedding_api_key = SecretStr("sk-test")
        mock_settings.embedding_dim = configured_dim

        mock_model = MagicMock()
        mock_model.aembed_query = AsyncMock(return_value=[0.1] * probe_dim)
        mock_provider = MagicMock()
        mock_provider.get_embedding_model.return_value = mock_model
        mock_factory.create.return_value = mock_provider

        return EmbeddingService(), mock_model

    @pytest.mark.asyncio
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_small_model_detects_1536_dims(self, mock_settings, mock_factory):
        """text-embedding-3-small produces 1536 dims — detected and stored correctly."""
        service, _ = self._make_service_and_factory(
    ...
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_large_model_detects_3072_dims(self, mock_settings, mock_factory):
        """text-embedding-3-large produces 3072 dims — detected and stored correctly."""
        service, _ = self._make_service_and_factory(
            mock_settings, mock_factory,
            model_name="text-embedding-3-large",
            probe_dim=3072,
        ...
        assert service._embedding_dim == 3072
        assert service._initialized is True

    @pytest.mark.asyncio
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_mismatch_raises_descriptive_error(self, mock_settings, mock_factory):
        """Model returns 3072 dims but config says 1536 — raises ValueError, no partial state."""
        service, _ = self._make_service_and_factory(
            mock_settings, mock_factory,
            model_name="text-embedding-3-large",
            probe_dim=3072,       # what the model actually returns
            configured_dim=1536,  # wrong config
        )
        ...
        assert "3072" in error_msg
        assert "1536" in error_msg
        assert "EMBEDDING_DIM" in error_msg
        # Service must NOT be marked initialized after a mismatch
        assert service._initialized is False
        assert service._embedding_dim == 0

    @pytest.mark.asyncio
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_mismatch_reversed_raises_error(self, mock_settings, mock_factory):
        """Model returns 1536 dims but config says 3072 — also raises ValueError."""
        service, _ = self._make_service_and_factory(
            mock_settings, mock_factory,
            model_name="text-embedding-3-small",
            probe_dim=1536,
    ...
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_store_rejects_wrong_dimension_vector(self, mock_settings, mock_factory):
        """store_tool_embedding raises if the vector dim doesn't match _embedding_dim."""
        service, _ = self._make_service_and_factory(
            mock_settings, mock_factory,
            model_name="text-embedding-3-small",
            probe_dim=1536,
        ...
        # Try to store a 3072-dim vector into a service initialized for 1536
        wrong_dim_embedding = [0.1] * 3072

        with pytest.raises(ValueError) as exc_info:
            service.store_tool_embedding(mock_db, "tool-1", wrong_dim_embedding, "text-embedding-3-large")
    ...
    @patch("mcpgateway.services.embedding_service.EmbeddingProviderFactory")
    @patch("mcpgateway.services.embedding_service.settings")
    async def test_store_accepts_correct_dimension_vector(self, mock_settings, mock_factory):
        """store_tool_embedding succeeds when dimension matches _embedding_dim."""
        service, _ = self._make_service_and_factory(
            mock_settings, mock_factory,
            model_name="text-embedding-3-small",
            probe_dim=1536,
        ...
        # Should not raise
        service.store_tool_embedding(mock_db, "tool-1", correct_embedding, "text-embedding-3-small")
