...
@pytest.mark.asyncio
async def test_admin_team_id_scopes_to_team_and_keeps_public(server_service, monkeypatch):
    ...
    monkeypatch.setattr("mcpgateway.services.server_service._get_registry_cache", lambda: mock_cache)

    mock_paginate = AsyncMock(return_value=([], None))
    monkeypatch.setattr("mcpgateway.services.server_service.unified_paginate", mock_paginate)
    ...
    assert "servers.team_id = 'team-1'" in compiled
    # ... but globally-public rows from any team are still ORed in.
    assert "servers.visibility = 'public'" in compiled
