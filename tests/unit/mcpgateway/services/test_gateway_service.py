...
class TestGatewayService:
    ...
    async def test_register_gateway_preserves_mcp_apps_metadata(self, gateway_service, monkeypatch):
        ...
        gateway_service._initialize_gateway = AsyncMock(return_value=({"tools": {}, "resources": {}}, [tool], [resource], [], [], None))
        gateway_service._notify_gateway_added = AsyncMock()
    ...
    async def test_register_gateway_skips_unsafe_federated_mcp_apps_metadata(self, gateway_service, monkeypatch):
        ...
        gateway_service._check_gateway_uniqueness = MagicMock(return_value=None)
        gateway_service._initialize_gateway = AsyncMock(return_value=({"tools": {}, "resources": {}}, [unsafe_tool, valid_tool], [], [], [], None))
        gateway_service._notify_gateway_added = AsyncMock()
        gateway_service.convert_gateway_to_read = MagicMock(return_value=MagicMock())
    ...
    async def test_register_gateway_skips_unsafe_federated_ui_resource_metadata(self, gateway_service, monkeypatch):
        ...
        gateway_service._check_gateway_uniqueness = MagicMock(return_value=None)
        gateway_service._initialize_gateway = AsyncMock(return_value=({"tools": {}, "resources": {}}, [], [unsafe_resource, valid_resource], [], [], None))
        gateway_service._notify_gateway_added = AsyncMock()
        gateway_service.convert_gateway_to_read = MagicMock(return_value=MagicMock())
    ...
    async def test_initialize_gateway_self_heals_stored_credential_with_invisible_char(self, gateway_service):
        ...
        gateway_service.connect_to_sse_server = AsyncMock(return_value=({}, [], [], [], [], None))
    ...
    async def test_update_gateway_persists_ca_and_mtls_fields(self, gateway_service, mock_gateway, test_db):
        ...
        gateway_service._initialize_gateway = AsyncMock(return_value=({"tools": {"subscribe": True}}, [], [], [], [], None))
    ...
    async def test_update_gateway_url_change_with_tools(self, gateway_service, mock_gateway, test_db):
        ...
        gateway_service._initialize_gateway = AsyncMock(return_value=({"tools": {"listChanged": True}}, new_tools, [], [], [], None))
        gateway_service._notify_gateway_updated = AsyncMock()
    ...
    async def test_set_gateway_state(self, gateway_service, mock_gateway, test_db):
        ...
        gateway_service._initialize_gateway = AsyncMock(return_value=({"prompts": {}}, [], [], [], [], None))
    ...
    async def test_set_gateway_state_authcode_partial_result_still_cleans(self, gateway_service, mock_gateway, test_db):
        ...
        gateway_service._notify_gateway_activated = AsyncMock()
        gateway_service._initialize_gateway = AsyncMock(return_value=({}, [new_tool], [], [], [], None))
    ...
    @pytest.mark.parametrize("advertised", ["omitted", "empty", "populated"])
    async def test_catalog_discovery_uses_capability_presence(self, gateway_service, connector, advertised):
        ...
        with patch("mcpgateway.services.gateway_service.mcp_proxy_client", return_value=proxy):
            _, _, resources, prompts, _, _ = await getattr(gateway_service, connector)("https://test.example.com/mcp")
    ...
    async def test_initialize_gateway_with_resources_and_prompts(self, gateway_service):
        ...
        with (
            ...
            # Execute
            capabilities, tools, resources, prompts, validation_errors, _ = await gateway_service._initialize_gateway("http://test.example.com", {"Authorization": "Bearer token"}, "SSE")
    ...
    async def test_initialize_gateway_oauth_auth_code_returns_empty(self, gateway_service):
        ...
        gateway_service.connect_to_sse_server = AsyncMock(return_value=({"tools": {}}, [], [], [], [], None))

        oauth_config = {"grant_type": "authorization_code"}
        capabilities, tools, resources, prompts, validation_errors, _ = await gateway_service._initialize_gateway(
...
class TestGatewayRefresh:
    ...
    async def test_refresh_gateway_success_all_changed(self, gateway_service, mock_gateway_with_relations, mock_db_session):
        ...
        with patch("mcpgateway.services.gateway_service.fresh_db_session", return_value=mock_db_session):
            ...
            gateway_service._initialize_gateway = AsyncMock(return_value=({}, new_tools, new_resources, new_prompts, [], None))  # capabilities
    ...
    async def test_deprecated_flat_meta_honoured_on_resource_ingest_paths(self, gateway_service):
        ...
        with patch("mcpgateway.services.gateway_service.mcp_proxy_client", return_value=mock_proxy_cm):
            _capabilities, _tools, resources, _prompts, _errors, _ = await gateway_service.connect_to_sse_server("https://test.example.com")
    ...
    async def test_connect_to_sse_server_without_validation_success(self, gateway_service):
        ...
        with patch("mcpgateway.services.gateway_service.mcp_proxy_client", return_value=mock_proxy_cm):
            # Execute
            capabilities, tools, resources, prompts, validation_errors, _ = await gateway_service._connect_to_sse_server_without_validation("http://test.com")
...
async def test_register_gateway_auth_value_decode_failure(gateway_service, monkeypatch):
    ...
    gateway_service._check_gateway_uniqueness = MagicMock(return_value=None)
    gateway_service._initialize_gateway = AsyncMock(return_value=({"tools": {}}, [], [], [], [], None))
    gateway_service._notify_gateway_added = AsyncMock()
    gateway_service.convert_gateway_to_read = MagicMock(return_value=MagicMock())
...
async def test_register_gateway_reassigns_orphaned_resource(gateway_service, monkeypatch):
    ...
    gateway_service._check_gateway_uniqueness = MagicMock(return_value=None)
    gateway_service._initialize_gateway = AsyncMock(return_value=({"tools": {}}, [], [resource], [prompt], [], None))
    gateway_service._notify_gateway_added = AsyncMock()
    gateway_service.convert_gateway_to_read = MagicMock(return_value=MagicMock())
...
async def test_connect_to_sse_server_without_validation_fallbacks(monkeypatch):
    ...
    capabilities, tools, resources, prompts, validation_errors, _ = await service._connect_to_sse_server_without_validation("http://server")
...
async def test_connect_to_streamablehttp_server_resources_and_prompts(monkeypatch):
    ...
    capabilities, tools, resources, prompts, validation_errors, _ = await service.connect_to_streamablehttp_server("http://server", ca_certificate=b"cert")
...
async def test_connect_to_sse_server_resources_and_prompts(monkeypatch):
    ...
    capabilities, tools, resources, prompts, validation_errors, _ = await service.connect_to_sse_server("http://server")
...
async def test_fetch_tools_after_oauth_streamablehttp(gateway_service, monkeypatch):
    ...
    monkeypatch.setattr("mcpgateway.services.token_storage_service.TokenStorageService", DummyTokenStorage)
    gateway_service.connect_to_streamablehttp_server = AsyncMock(return_value=({"tools": {}}, [], [], [], [], None))
...
async def test_fetch_tools_after_oauth_cleanup_and_adds_items(gateway_service, monkeypatch):
    ...
    gateway_service._connect_to_sse_server_without_validation = AsyncMock(
        return_value=({"resources": True, "prompts": True}, [SimpleNamespace(name="keep-tool")], [SimpleNamespace(uri="keep://res")], [SimpleNamespace(name="keep-prompt")], [], None)
    )
...
class TestInitializeGateway:
    ...
    async def test_oauth_auth_code_skips_connection(self, gateway_service):
        ...
        caps, tools, resources, prompts, validation_errors, _ = await gateway_service._initialize_gateway(
            url="http://example.com",
            auth_type="oauth",
            oauth_config={"grant_type": "authorization_code"},
            oauth_auto_fetch_tool_flag=False,
    ...
    async def test_oauth_client_credentials_success(self, gateway_service):
        ...
        gateway_service.connect_to_sse_server = AsyncMock(return_value=({"tools": {"listChanged": True}}, [], [], [], [], None))
        caps, tools, resources, prompts, validation_errors, _ = await gateway_service._initialize_gateway(
            url="http://example.com",
            auth_type="oauth",
            oauth_config={"grant_type": "client_credentials"},
    ...
    async def test_sse_transport(self, gateway_service):
        gateway_service.connect_to_sse_server = AsyncMock(return_value=({"tools": {}}, [SimpleNamespace(name="t1")], [], [], [], None))
        caps, tools, resources, prompts, validation_errors, _ = await gateway_service._initialize_gateway(url="http://example.com", transport="SSE")
        assert len(tools) == 1
        gateway_service.connect_to_sse_server.assert_awaited_once()

    @pytest.mark.asyncio
    async def test_streamablehttp_transport(self, gateway_service):
        gateway_service.connect_to_streamablehttp_server = AsyncMock(return_value=({"tools": {}}, [SimpleNamespace(name="t1")], [], [], [], None))
        caps, tools, resources, prompts, validation_errors, _ = await gateway_service._initialize_gateway(url="http://example.com", transport="StreamableHTTP")
        assert len(tools) == 1
    ...
    async def test_oauth_auth_code_with_flag(self, gateway_service):
        ...
        gateway_service.connect_to_sse_server = AsyncMock(return_value=({"tools": {}}, [SimpleNamespace(name="t1")], [], [], [], None))
        caps, tools, _, _, _, _ = await gateway_service._initialize_gateway(
            url="http://example.com",
            auth_type="oauth",
            oauth_config={"grant_type": "authorization_code"},
...
class TestRefreshGatewayToolsResourcesPrompts:
    ...
    async def test_validation_errors_propagated(self, gateway_service):
        ...
        gateway_service._initialize_gateway = AsyncMock(return_value=({}, [], [], [], validation_errors, None))
        result = await gateway_service._refresh_gateway_tools_resources_prompts("gw-1", gateway=gw)
...
class TestUpdateGatewayAdvanced:
    ...
    async def test_update_passthrough_headers_list(self, gateway_service, mock_gateway, monkeypatch):
        ...
        monkeypatch.setattr(gateway_service, "_initialize_gateway", AsyncMock(return_value=({"tools": {}}, [], [], [], [], None)))
    ...
    async def test_update_stale_tools_cleaned_up(self, gateway_service, mock_gateway, monkeypatch):
        ...
        monkeypatch.setattr(gateway_service, "_initialize_gateway", AsyncMock(return_value=({"tools": {}}, [new_tool], [], [], [], None)))
...
class TestSetGatewayStateActivation:
    ...
    async def test_activate_with_new_resources_and_prompts(self, gateway_service, mock_gateway, monkeypatch):
        ...
        monkeypatch.setattr(gateway_service, "_initialize_gateway", AsyncMock(return_value=({"tools": {}, "resources": {}, "prompts": {}}, [new_tool], [new_resource], [new_prompt], [], None)))
        monkeypatch.setattr(gateway_service, "_create_db_tool", MagicMock(return_value=MagicMock()))
        monkeypatch.setattr("mcpgateway.services.gateway_service._get_registry_cache", lambda: MagicMock(invalidate_gateways=AsyncMock()))
        monkeypatch.setattr("mcpgateway.services.gateway_service._get_tool_lookup_cache", lambda: MagicMock(invalidate_gateway=AsyncMock()))
        monkeypatch.setattr("mcpgateway.cache.admin_stats_cache.admin_stats_cache", MagicMock(invalidate_tags=AsyncMock()))
