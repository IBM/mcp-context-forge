/**
 * Unit tests for pagination state management functions.
 *
...
// ---------------------------------------------------------------------------
// Local implementations of getPaginationParams and buildTableUrl.
// These mirror the inline <script> definitions in admin.html.
// Defined here as pure functions to test the logic without loading admin.js.
// ---------------------------------------------------------------------------
function getPaginationParams(tableName) {
...
function buildTableUrl(tableName, baseUrl, additionalParams = {}) {
  ...
  if (namespacedQuery) url.searchParams.set("q", namespacedQuery);
  if (namespacedTags) url.searchParams.set("tags", namespacedTags);
...
describe("getTeamsCurrentPaginationState", () => {
  ...
  test("handles non-numeric values in URL params", () => {
    window.history.replaceState({}, "", "/admin?teams_page=abc&teams_size=xyz");
    const state = getTeamsCurrentPaginationState();
    expect(state).toEqual({
      page: 1,
      perPage: 10,
...
describe("getPaginationParams namespace isolation", () => {
  test("each table reads only its own namespaced URL params", () => {
    ...
    const servers = getPaginationParams("servers");
    const tools = getPaginationParams("tools");
  ...
  test("all five section namespaces are independent", () => {
    ...
    sections.forEach((name, i) => {
      const state = getPaginationParams(name);
...
describe("buildTableUrl namespace isolation", () => {
  test("builds URL using only the specified table's params", () => {
    ...
    const serversUrl = buildTableUrl("servers", "/admin/servers/partial");
    const toolsUrl = buildTableUrl("tools", "/admin/tools/partial");
...
describe("pagination loadPage swapStyle (#3396)", () => {
  ...
  test("outerHTML swapStyle is passed to htmx.ajax for table targets", () => {
    const { component, ajaxCalls } = createComponentWithAjaxSpy("outerHTML");
  ...
  test("innerHTML swapStyle is passed to htmx.ajax for non-table targets", () => {
    const { component, ajaxCalls } = createComponentWithAjaxSpy("innerHTML");
  ...
  test("swapStyle defaults to innerHTML when not explicitly set", () => {
    ...
    const { component, ajaxCalls } = createComponentWithAjaxSpy(defaultSwap);
    component.loadPage(2);
    expect(ajaxCalls[0].swap).toBe("innerHTML");
  });
});
