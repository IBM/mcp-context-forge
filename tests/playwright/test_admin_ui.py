...
class TestAdminUI:
    ...
    def test_navigate_between_tabs(self, admin_page: AdminPage, base_url: str):
        ...
        # Accept both /admin/ and /ui/ with or without trailing slash
        expect(admin_page.page).to_have_url(re.compile(f"{re.escape(base_url)}/(admin|ui)/?#catalog"))

        # Test tools tab
        admin_page.click_tools_tab()
        expect(admin_page.page).to_have_url(re.compile(f"{re.escape(base_url)}/(admin|ui)/?#tools"))

        # Test gateways tab
        admin_page.click_gateways_tab()
        expect(admin_page.page).to_have_url(re.compile(f"{re.escape(base_url)}/(admin|ui)/?#gateways"))
