...
class TestVersionPage:
    ...
    def test_database_status_indicator(self, version_page: VersionPage):
        ...
        if is_healthy:
            assert "Reachable" in badge_text or "✅" in badge_text, \
                "Healthy database should show 'Reachable' or checkmark"
    ...
    def test_cache_status_indicator(self, version_page: VersionPage):
        ...
        # If healthy, should contain positive indicators
        if is_healthy:
            assert "Connected" in badge_text or "✅" in badge_text, \
                "Healthy cache should show 'Connected' or checkmark"
    ...
    def test_support_bundle_card_displays(self, version_page: VersionPage):
        ...
        assert "/admin/support-bundle/generate" in href, \
            "Download button should link to support bundle endpoint"
    ...
    def test_bundle_contents_list_displays(self, version_page: VersionPage):
        ...
        assert version_page.verify_bundle_contents_visible(), \
            "All bundle content items should be visible"
...
class TestVersionPageIntegration:
    ...
    def test_version_data_consistency(self, version_page: VersionPage):
        ...
        assert version_page.get_app_name() == initial_app_name, \
            "App name should remain consistent"
        assert version_page.get_app_version() == initial_version, \
            "App version should remain consistent"
        assert version_page.get_python_version() == initial_python, \
            "Python version should remain consistent"
    ...
    def test_services_status_reflects_system_state(self, version_page: VersionPage):
        ...
        assert db_healthy or cache_healthy, \
            "At least one service should be healthy in test environment"
        ...
        if db_healthy:
            assert "green" in db_card_classes, \
                "Healthy database card should have green styling"
        
        cache_card_classes = version_page.cache_status_card.get_attribute("class")
        if cache_healthy:
            assert "green" in cache_card_classes, \
                "Healthy cache card should have green styling"