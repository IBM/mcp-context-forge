...
Advanced text chunking and splitting server with multiple strategies.
...
server = Server("chunker-server")


class ChunkTextRequest(BaseModel):
    """Request to chunk text."""
    text: str = Field(..., description="Text to chunk")
    chunk_size: int = Field(1000, description="Maximum chunk size in characters", ge=100, le=100000)
    chunk_overlap: int = Field(200, description="Overlap between chunks in characters", ge=0)
...
class ChunkMarkdownRequest(BaseModel):
    """Request to chunk markdown text with header awareness."""
    text: str = Field(..., description="Markdown text to chunk")
    headers_to_split_on: List[str] = Field(["#", "##", "###"], description="Headers to split on")
    chunk_size: int = Field(1000, description="Maximum chunk size", ge=100, le=100000)
    chunk_overlap: int = Field(100, description="Overlap between chunks", ge=0)


class SemanticChunkRequest(BaseModel):
    """Request for semantic chunking."""
    text: str = Field(..., description="Text to chunk semantically")
    min_chunk_size: int = Field(200, description="Minimum chunk size", ge=50)
    max_chunk_size: int = Field(2000, description="Maximum chunk size", ge=100, le=100000)
    similarity_threshold: float = Field(0.8, description="Similarity threshold for grouping", ge=0.0, le=1.0)


class SentenceChunkRequest(BaseModel):
    """Request for sentence-based chunking."""
    text: str = Field(..., description="Text to chunk by sentences")
    sentences_per_chunk: int = Field(5, description="Target sentences per chunk", ge=1, le=50)
    overlap_sentences: int = Field(1, description="Overlapping sentences", ge=0, le=10)


class FixedSizeChunkRequest(BaseModel):
    """Request for fixed-size chunking."""
    text: str = Field(..., description="Text to chunk")
    chunk_size: int = Field(1000, description="Fixed chunk size", ge=100, le=100000)
    overlap: int = Field(0, description="Overlap between chunks", ge=0)
    split_on_word_boundary: bool = Field(True, description="Split on word boundaries")


class AnalyzeTextRequest(BaseModel):
    """Request to analyze text for optimal chunking."""
...
# Initialize chunker (conditionally for testing)
try:
    chunker = TextChunker()
except Exception:
    chunker = None
...
async def handle_list_tools() -> list[Tool]:
    ...
    return [
        Tool(
            name="chunk_text",
            description="Chunk text using recursive character splitting",
            inputSchema={
                "type": "object",
                "properties": {
                    "text": {
                    ...
                    "chunking_strategy": {
                        "type": "string",
                        "enum": ["recursive", "semantic", "sentence", "fixed_size"],
                    ...
                    "separators": {
                        "type": "array",
                        "items": {"type": "string"},
                        "description": "Custom separators for splitting (optional)"
                    },
                    "preserve_structure": {
                        "type": "boolean",
                        "description": "Preserve document structure when possible",
                        "default": True
                    }
                },
                "required": ["text"]
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "text": {
                        "type": "string",
                        "description": "Markdown text to chunk"
                    },
                    "headers_to_split_on": {
                        "type": "array",
                        "items": {"type": "string"},
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "text": {
                        "type": "string",
                        "description": "Text to chunk by sentences"
                    },
                    "sentences_per_chunk": {
                        "type": "integer",
                        "description": "Target sentences per chunk",
                        "default": 5,
                        "minimum": 1,
                        "maximum": 50
                    },
                    "overlap_sentences": {
                        "type": "integer",
                        "description": "Overlapping sentences between chunks",
                        "default": 1,
                        "minimum": 0,
                        "maximum": 10
                    }
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    ...
                    "chunk_size": {
                        "type": "integer",
                        "description": "Fixed chunk size in characters",
                        "default": 1000,
                        "minimum": 100,
                        "maximum": 100000
                    },
                    ...
                    "split_on_word_boundary": {
                        "type": "boolean",
                        "description": "Split on word boundaries to avoid breaking words",
                        "default": True
                    }
                },
                "required": ["text"]
        ...
        Tool(
            ...
            inputSchema={
                ...
                "properties": {
                    "text": {
                        "type": "string",
                        "description": "Text to analyze for chunking recommendations"
                    }
                },
                "required": ["text"]
            }
...
async def handle_call_tool(name: str, arguments: dict[str, Any]) -> Sequence[TextContent | ImageContent | EmbeddedResource]:
    ...
    try:
        if chunker is None:
            result = {"success": False, "error": "Text chunker not available"}
        elif name == "chunk_text":
            request = ChunkTextRequest(**arguments)

            if request.chunking_strategy == "recursive":
                result = chunker.recursive_chunk(
                    text=request.text,
            ...
            elif request.chunking_strategy == "sentence":
                result = chunker.sentence_chunk(text=request.text)
            elif request.chunking_strategy == "fixed_size":
                ...
                result = {"success": False, "error": f"Unknown strategy: {request.chunking_strategy}"}

        elif name == "chunk_markdown":
            request = ChunkMarkdownRequest(**arguments)
            result = chunker.markdown_chunk(
                text=request.text,
                headers_to_split_on=request.headers_to_split_on,
                chunk_size=request.chunk_size,
                chunk_overlap=request.chunk_overlap
            )

        elif name == "semantic_chunk":
            request = SemanticChunkRequest(**arguments)
            result = chunker.semantic_chunk(
                text=request.text,
        ...
        elif name == "sentence_chunk":
            request = SentenceChunkRequest(**arguments)
            result = chunker.sentence_chunk(
                text=request.text,
                sentences_per_chunk=request.sentences_per_chunk,
                overlap_sentences=request.overlap_sentences
            )

        elif name == "fixed_size_chunk":
            request = FixedSizeChunkRequest(**arguments)
            result = chunker.fixed_size_chunk(
                text=request.text,
                chunk_size=request.chunk_size,
                overlap=request.overlap,
                split_on_word_boundary=request.split_on_word_boundary
            )

        elif name == "analyze_text":
            request = AnalyzeTextRequest(**arguments)
            result = chunker.analyze_text(text=request.text)
