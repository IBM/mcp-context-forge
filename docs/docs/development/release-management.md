...
```bash
make hadolint dockle grype-scan
```
...
| `grype-scan` | Vulnerability scan of the built image for OS packages and application dependencies |

**Acceptance criteria:** No HIGH or CRITICAL vulnerabilities in `grype-scan` output. No errors from `hadolint` (warnings are acceptable if documented). No failures from `dockle` at warn level.
...
The CI pipeline (`docker-scan.yml`) runs SBOM generation and a Grype vulnerability scan on the container image. Verify no critical or high vulnerabilities exist in the final image. For local verification, see also [Section 6.2](#62-containerfile-linting-and-image-scanning).
