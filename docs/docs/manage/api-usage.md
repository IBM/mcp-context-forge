...
| `400` | Active team with the same name already exists (platform admin caller) | `A team named 'Marketing' already exists` |
| `403` | `ALLOW_TEAM_CREATION=false` and caller is not admin | `Team creation is currently disabled` |
| `409` | Active team with the same name already exists (non-admin caller; purposefully generic so a team's existence cannot be probed) | `A team with the same name could not be created` |
