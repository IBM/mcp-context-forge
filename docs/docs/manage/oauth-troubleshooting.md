...
### Diagnosis steps
1.  **Check Gateway Configuration**: Verify the gateway's current `oauth_config["resource"]` value (via `GET /admin/gateways/{id}` or the Admin UI). If it is empty, the gateway is using the auto-derived gateway URL origin as a fallback.
...
-   **Multi-tenant Entra ID, wrong tenant**: If the token is being issued for the wrong audience in a multi-tenant setup, set the `oauth_config["resource"]` explicitly to your Application ID URI (e.g., `api://{your-app-id}`).
