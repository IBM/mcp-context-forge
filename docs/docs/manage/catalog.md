...
catalog_servers:
  ...
  - id: "websocket-server"
    ...
    transport: "WEBSOCKET"  # DEPRECATED: sunsets 2027-01-18
    auth_type: "API Key"
    provider: "Internal Platform"
...
## Bundled Icon Maintenance

Catalog entries with a bundled logo use a local path such as
`/static/catalog-icons/github.png`. These assets are generated by maintainers
before release; gateway requests never fetch remote icon URLs.

To reduce excess transparent padding in existing bundled assets without any
network access, run:
...
| `transport` | string | No | Transport type: `SSE`, `STREAMABLEHTTP`, or `WEBSOCKET` (auto-detected if not specified). The `WEBSOCKET` value is deprecated. It sunsets on 2027-01-18. |
