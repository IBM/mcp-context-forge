...
The gRPC target is validated against the platform SSRF settings before any channel is opened. The source fallback uses `SSRF_ALLOW_LOCALHOST=false` and `SSRF_ALLOW_PRIVATE_NETWORKS=false`; this project's intranet release images explicitly override both to `true`. Stricter deployments should set both flags to `false`, then add only the required ranges to `SSRF_ALLOWED_NETWORKS` (see the [1.0.0-RC3 upgrade notes](../manage/upgrade-to-1.0.0-rc3.md) for the full table of source defaults).
