...
The `cforge` CLI is installed with the ContextForge package:
...
- **gateway**: Defines the ContextForge configuration - can use a pre-built image or build from a Git repository
...
PostgreSQL and Redis are **automatically deployed** with the ContextForge stack using hardcoded defaults:
