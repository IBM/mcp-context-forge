...
!!! note "Rust MCP runtime sidecar fully removed"
    The Rust MCP runtime sidecar, Rust A2A runtime sidecar, and
    `ValidationMiddleware` have been fully removed from the codebase.
    Use the default Python runtime paths and endpoint-level validation.
