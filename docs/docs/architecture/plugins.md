...
| `language` | `string` | Implementation language | `"python"`, `"typescript"`, `"go"`, `"java"` |
...
(see [Security Architecture](security-features.md)), plugin
...
| Hook Type | Direct Path | Python Path |
|-----------|-------------|-------------|
| `TOOL_PRE_INVOKE` | Runs in Python during `/resolve`; results applied by executor | Runs normally in Python |
...
- **Pre-invoke plugins** work in the Python path. Modifications to
  arguments and headers are forwarded through the execution plan.
- **Post-invoke plugins** are fully supported but cause some
  `tools/call` requests to go through the standard Python path, bypassing
  any direct execution optimization. This is by design — post-invoke hooks
  need access to the upstream response.
...
Dependencies with a `preInvokePlanCacheEnabled` flag enable plan caching
per tool call signature. When `hasPreInvokeHooks` is
