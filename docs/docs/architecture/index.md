...
**16. Secure Code Execution & Virtual Tool Filesystem**

- First-class `code_execution` virtual servers with synthetic meta-tools: `shell_exec` and `fs_browse`
- Policy-driven sandbox runtime controls (runtime, time/memory/cpu/network/file/disk/rate limits)
- Virtual filesystem projection (`/tools`, `/skills`, `/scratch`, `/results`) with mount-rule filtering
- Optional tokenization policy and skill governance (`skills_scope`, approval workflows)
- Operational endpoints for runs, sessions, security events, and replay
...
- [Secure Code Execution and Virtual Tool Filesystem](code-execution-virtual-tool-filesystem.md) - Architecture for MCP Code Mode and sandboxed virtual execution
