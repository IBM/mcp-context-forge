...
remains Python. For the broader
high-performance view, see the
[Performance Architecture Diagram](performance-architecture.md).

The target-state modular runtime architecture is documented separately
in [Modular Runtime Architecture](modular-design.md).

The implementation-ready contract for building new modules lives in the
...
ContextForge leverages Rust-powered components for maximum throughput and minimal latency, including fast JSON libraries, Rust-based validation engines, and native HTTP servers.
