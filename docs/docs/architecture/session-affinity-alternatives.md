...
**Follow-up experiment — `hash $http_authorization` works end-to-end.**
The branch [`experiment/sticky-lb-auth-hash`](https://github.com/IBM/mcp-context-forge/tree/experiment/sticky-lb-auth-hash) (built on the previous experiment) swapped the nginx hash key from `Mcp-Session-Id` to `Authorization`. Routing is stable from the very first request because the client already holds the bearer token before sending `initialize` — the LB hashes the same string on the bind request and on every follow-up, so they always land on the same pod. Five tests passed: counter reproducer (strict monotonic 1..25), cross-user isolation (two tokens → two pods, independent counters), same-user multi-session (three sessions pinned to one pod, each independent), pod-kill failover (sessions on the dead pod are lost; consistent hash routes the same token to a remaining live pod; fresh `initialize` rebinds cleanly), and a per-user-token benchmark of **352 RPS at 0% failures with p99 530 ms** on 3 single-worker pods. Per-worker efficiency is **~21× the affinity-layer baseline** (#4987 ran 397 RPS on 72 workers; this ran 352 RPS on 3). The benchmark required a small env-gated patch to `tests/loadtest/locustfile_mcp_protocol.py` so each Locust user mints its own JWT (`BENCHMARK_UNIQUE_TOKENS=1`) — without it, the locustfile's shared singleton token collapses every simulated user onto one backend and tells you nothing about distribution. Trade-offs specific to user-pinning (heavy-user concentration, session loss on token refresh) are mitigated by per-`$http_authorization` nginx rate limiting and by typical JWT lifetimes. See the [experiment write-up](https://github.com/IBM/mcp-context-forge/blob/experiment/sticky-lb-auth-hash/docs/docs/architecture/experiments/sticky-lb-auth-hash.md) for the full breakdown including the comparison table and production-acceptability notes. Net takeaway: **Approach 1 ships as a config-only change after all**, provided the LB hash key is `Authorization` rather than `Mcp-Session-Id`.
