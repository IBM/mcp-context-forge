...
**Container Security Hardening**: Our containerized deployments follow security best practices including multi-stage builds, minimal base images (UBI Micro) with the latest updates, non-root user execution, read-only filesystems, and container scanning with tools like Grype, OSV-Scanner, and SBOM-based review.
...
- **Dependency Vulnerability Scanning**: OSV-Scanner, Grype, pip-audit, npm audit, cargo audit (Rust), govulncheck (Go), and GitHub dependency review with license policy enforcement
- **Container Security**: Dockerfile linting, SBOM generation, Dockle where used, and Grype for vulnerability scanning
...
flowchart TD
    ...
    W --> W14[make grype-scan - Container Vulnerability]
