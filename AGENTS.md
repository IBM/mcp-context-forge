...
- Admin routes over records with no team association require an unrestricted platform-admin token (`token_teams is None`). Do not re-implement this check; use shared helpers in `mcpgateway/middleware/rbac.py`: `require_global_admin_permission()` for whole-endpoint guards, `require_unrestricted_platform_admin()` for conditional call sites, and `Depends(require_global_admin_scope_dep)` for router-level dependency guards. When stacking with an existing `@require_permission(...)` decorator, place the global-record guard above it so the fine-grained permission remains declared in source. Every such route must be classified in `tests/unit/mcpgateway/test_global_record_scope.py`; the drift guard fails the build otherwise.
