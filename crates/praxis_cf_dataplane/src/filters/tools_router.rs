...
impl ToolsRouterFilter {
    ...
    pub fn from_config(config: &serde_yaml::Value) -> Result<Box<dyn HttpFilter>, FilterError> {
        let cfg: Config =
            serde_yaml::from_value(config.clone()).map_err(|e| FilterError::from(e.to_string()))?;
...
impl HttpFilter for ToolsRouterFilter {
    ...
    async fn on_request(
        ...
        let virtual_server_id = ctx
            .get_metadata("mcp.virtual_server_id")
