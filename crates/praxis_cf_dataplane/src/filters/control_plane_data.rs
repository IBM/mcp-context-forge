...
use praxis_filter::{FilterAction, FilterError, HttpFilter, HttpFilterContext, Rejection, Request};
...
impl ControlPlaneDataFilter {
    ...
    fn unfinished_auth_action(&self, req: &Request) -> FilterAction {
        let _pending_token = self.extract_token(req);
...
impl HttpFilter for ControlPlaneDataFilter {
    ...
    async fn on_request(
        ...
        let _virtual_server_id = self.extract_virtual_server_id(req).ok_or_else(|| {
        ...
        Ok(self.unfinished_auth_action(req))
    }
}
...
mod tests {
    ...
    fn filter() -> ControlPlaneDataFilter {
        ControlPlaneDataFilter {
            grpc_endpoint: "http://control-plane.test".to_string(),
        }
    }
    ...
    fn request(authorization: Option<&str>) -> Request {
        ...
        Request {
            headers,
            method: Method::POST,
            uri: Uri::from_static("/server/vs-123/mcp"),
        }
    }
    ...
    fn test_extract_token() {
        let filter = filter();
        let req = request(Some("Bearer test-token-123"));
    ...
    #[test]
    fn control_plane_data_rejects_missing_bearer() {
        let action = filter().unfinished_auth_action(&request(None));

        assert!(matches!(action, FilterAction::Reject(rejection) if rejection.status == 401));
    }

    #[test]
    fn control_plane_data_rejects_malformed_bearer() {
        let action = filter().unfinished_auth_action(&request(Some("Basic malformed-value")));

        assert!(matches!(action, FilterAction::Reject(rejection) if rejection.status == 401));
    }

    #[test]
    fn control_plane_data_rejects_syntactically_valid_bearer() {
        let action =
            filter().unfinished_auth_action(&request(Some("Bearer syntactically-valid-value")));

        assert!(matches!(action, FilterAction::Reject(rejection) if rejection.status == 401));
    }

    #[test]
    fn control_plane_data_stderr_probe() {
        let sentinel = std::env::var("PRAXIS_BEARER_SENTINEL").unwrap_or_default();
        if sentinel.is_empty() {
            return;
        }

        let action = filter().unfinished_auth_action(&request(Some(&format!("Bearer {sentinel}"))));

        assert!(matches!(action, FilterAction::Reject(rejection) if rejection.status == 401));
    }

    #[test]
    fn control_plane_data_does_not_disclose_bearer_sentinels() {
        const PREFIX: &str = "TASK1_BEARER_PREFIX_6F1B";
        const SUFFIX: &str = "_TASK1_BEARER_SUFFIX_A92C";
        let sentinel = format!("{PREFIX}MIDDLE{SUFFIX}");
        let output = Command::new(std::env::current_exe().unwrap())
            .args([
                "--exact",
                "filters::control_plane_data::tests::control_plane_data_stderr_probe",
                "--nocapture",
            ])
            .env("PRAXIS_BEARER_SENTINEL", &sentinel)
            .output()
            .unwrap();
