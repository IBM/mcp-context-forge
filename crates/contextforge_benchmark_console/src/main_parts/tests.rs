...
fn discovers_suite_metadata_with_description() {
    ...
    std::fs::create_dir_all(tempdir.join("crates/contextforge_benchmark_runner/assets/scenarios"))
        .unwrap();
    std::fs::write(
        tempdir.join("crates/contextforge_benchmark_runner/assets/scenarios/example-suite.toml"),
        r#"
[suite]
...
fn suite_navigation_is_blocked_outside_suite_focused_views() {
    ...
    handle_normal_mode(&mut app, KeyEvent::from(KeyCode::Char('j')), &root).unwrap();
...
fn start_command_capture_resets_run_monitor_state() {
    ...
    assert!(
        app.log_lines[0]
            .text
            .contains("Started command inside console")
    );
}
use super::*;
