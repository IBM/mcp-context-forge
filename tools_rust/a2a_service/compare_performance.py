...
RUST_A2A_URL = "http://127.0.0.1:8790/invoke"
...
async def run_rust_lane(
    ...
    # Call the standalone Rust A2A HTTP service (low-level /invoke endpoint).
    # Convert benchmark payloads to the Rust JSON DTO contract.
    now = datetime.now(timezone.utc)
    requests: list[dict[str, Any]] = []
    for idx, payload in enumerate(payloads):
        # Match Python A2A request_data selection logic (good enough for stub server).
        if payload["agent_type"] in ["generic", "jsonrpc"] or str(payload["base_url"]).endswith("/"):
            request_data = {
                "jsonrpc": "2.0",
                "method": payload["parameters"].get("method", "message/send"),
                "params": payload["parameters"].get("params", payload["parameters"]),
                "id": 1,
            }
        else:
            request_data = {
                ...
                "protocol_version": payload["agent_protocol_version"],
            }
        ...
        requests.append(
            {
                ...
                "auth_headers_encrypted": payload.get("auth_value_encrypted"),
                "auth_query_params_encrypted": payload.get("auth_query_params_encrypted"),
            }
        )
    ...
    if len(results) != len(payloads):
        raise AssertionError(f"Rust returned {len(results)} results for {len(payloads)} payloads")
...
async def _async_main(selected_scenarios: set[str], output: Path | None) -> int:
    ...
    # Caller is responsible for starting the Rust A2A HTTP service separately.
