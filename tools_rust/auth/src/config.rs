...
pub struct AuthConfig {
    ...
    #[arg(
        long,
        env = "CONTEXTFORGE_AUTH_LISTEN_HTTP",
    ...
    #[arg(
        long,
        env = "CONTEXTFORGE_AUTH_EXPERIMENTAL_DIRECT_AUTH",
    ...
    #[arg(
        long,
        env = "CONTEXTFORGE_AUTH_SHADOW_COMPARE_DIRECT_AUTH",
        default_value_t = false
    )]
...
impl AuthConfig {
    pub fn listen_addr(&self) -> Result<SocketAddr, String> {
        self.listen_http.parse().map_err(|err| {
            format!(
                "invalid CONTEXTFORGE_AUTH_LISTEN_HTTP '{}': {err}",
                self.listen_http
            )
        })
    }
