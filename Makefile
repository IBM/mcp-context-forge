...
COMPOSE_CMD_MONITOR := $(shell \
	if [ "$${CONTAINER_RUNTIME:-}" = "podman" ] && command -v podman &>/dev/null && podman compose version &>/dev/null 2>&1; then \
		echo "podman compose"; \
	elif [ "$${CONTAINER_RUNTIME:-}" = "docker" ] && command -v docker &>/dev/null && docker compose version &>/dev/null 2>&1; then \
		echo "docker compose"; \
...
ifeq ($(strip $(COMPOSE_CMD)),)
  ifeq ($(strip $(CONTAINER_RUNTIME)),podman)
    COMPOSE_CMD := $(shell command -v podman >/dev/null 2>&1 && podman compose version >/dev/null 2>&1 && echo "podman compose" || true)
  endif
  ifeq ($(strip $(CONTAINER_RUNTIME)),docker)
    COMPOSE_CMD := $(shell command -v docker >/dev/null 2>&1 && docker compose version >/dev/null 2>&1 && echo "docker compose" || true)
  endif
...
# help: benchmark - Run the self-contained benchmark bundle. Use MODE=list|run|validate|smoke|check-runtime|report|compare, SCENARIO=<name>, ALL=1, RUN=<saved run dir>, CLEAN=1, BENCHMARK_ARGS="..."
.PHONY: benchmark

SCENARIO ?= modular-design-300
MODE ?= run
ALL ?= 0
RUN ?=
CLEAN ?= 1
...
benchmark:
	@MODE="$(MODE)" SCENARIO="$(SCENARIO)" ALL="$(ALL)" RUN="$(RUN)" CLEAN="$(CLEAN)" BENCHMARK_ARGS="$(BENCHMARK_ARGS)" CONTAINER_RUNTIME="$${CONTAINER_RUNTIME:-podman}" $(VENV_PYTHON) -c "exec(\"\"\"import os\nimport shlex\nimport shutil\nimport subprocess\nimport sys\nfrom pathlib import Path\n\nmode = os.environ.get('MODE', 'run').strip()\nscenario = os.environ.get('SCENARIO', 'modular-design-300').strip()\nall_flag = os.environ.get('ALL', '0').strip().lower() in {'1', 'true', 'yes'}\nrun_path = os.environ.get('RUN', '').strip()\nclean = os.environ.get('CLEAN', '1').strip().lower() in {'1', 'true', 'yes'}\nextra_args = shlex.split(os.environ.get('BENCHMARK_ARGS', ''))\ncontainer_runtime = os.environ.get('CONTAINER_RUNTIME', 'podman').strip() or 'podman'\n\nrun_modes = {'run', 'validate', 'smoke', 'check-runtime'}\nreport_modes = {'report', 'compare'}\nvalid_modes = {'list', *run_modes, *report_modes}\nif mode not in valid_modes:\n    supported = ', '.join(sorted(valid_modes))\n    raise SystemExit(f'Unsupported MODE={mode!r}. Use one of: {supported}')\nif all_flag and mode in report_modes:\n    raise SystemExit('ALL=1 cannot be combined with MODE=report or MODE=compare')\nif all_flag and mode == 'list':\n    raise SystemExit('ALL=1 is not used with MODE=list')\nif mode in report_modes and not run_path:\n    raise SystemExit(f'MODE={mode} requires RUN=reports/benchmarks/<run-dir>')\n\ndef cleanup() -> None:\n    print('Cleaning benchmark containers and artifacts...')\n    engine = container_runtime\n    if not engine:\n        engine = 'docker' if shutil.which('docker') else 'podman'\n\n    def run(cmd, timeout=10):\n        try:\n            return subprocess.run(cmd, capture_output=True, text=True, timeout=timeout, check=False)\n        except subprocess.TimeoutExpired:\n            return None\n        except FileNotFoundError:\n            return False\n\n    removed_pods = 0\n    removed_containers = 0\n    leftover_containers = 0\n    if engine == 'podman':\n        pod_list = run(['podman', 'pod', 'ps', '-a', '--format', '{{.Name}}'])\n        if pod_list is None:\n            print(\"Skipping pod cleanup because 'podman pod ps -a' timed out.\")\n        elif pod_list is not False:\n            pods = [line.strip() for line in (pod_list.stdout or '').splitlines() if line.strip().startswith('bench-')]\n            for pod in pods:\n                pod_rm = run(['podman', 'pod', 'rm', '-f', pod], timeout=20)\n                if pod_rm is not None and pod_rm is not False and pod_rm.returncode == 0:\n                    removed_pods += 1\n\n    container_list = run([engine, 'ps', '-a', '--format', '{{.Names}}'])\n    if container_list is None:\n        print(f\"Skipping container cleanup because '{engine} ps -a' timed out.\")\n    elif container_list is False:\n        print(f\"Skipping container cleanup because '{engine}' is not available.\")\n    else:\n        containers = [line.strip() for line in (container_list.stdout or '').splitlines() if line.strip().startswith('bench-')]\n        for name in reversed(containers):\n            rm_result = run([engine, 'rm', '-f', name], timeout=20)\n            if rm_result is not None and rm_result is not False and rm_result.returncode == 0:\n                removed_containers += 1\n            else:\n                leftover_containers += 1\n\n    if removed_pods or removed_containers:\n        print(f'Removed {removed_pods} benchmark pod(s) and {removed_containers} benchmark container(s).')\n    elif leftover_containers:\n        print(f'Left {leftover_containers} benchmark container(s) in place; cleanup will rely on per-run project names.')\n    else:\n        print('No benchmark containers found.')\n\n    reports_dir = Path('reports/benchmarks')\n    for pattern in [\n        '_runtime_staging/all-scenarios_*',\n        '_runtime_staging/modular-design-300_*',\n        '_runtime_staging/a2a-invoke-300_*',\n        '_runtime_staging/shared_stacks',\n        '_runtime_staging/source_checkouts',\n        'all-scenarios_*',\n        'modular-design-300_*',\n        'a2a-invoke-300_*',\n    ]:\n        for path in reports_dir.glob(pattern):\n            if path.is_dir():\n                shutil.rmtree(path, ignore_errors=True)\n            else:\n                path.unlink(missing_ok=True)\n    print('Removed benchmark artifacts under reports/benchmarks.')\n\nargs = ['-m', 'benchmarks.contextforge']\nif mode == 'list':\n    args.append('--list')\nelif mode in run_modes:\n    if all_flag:\n        args.append('--all')\n    else:\n        args.extend(['--scenario', scenario])\n    if mode == 'validate':\n        args.append('--validate')\n    elif mode == 'smoke':\n        args.append('--smoke')\n    elif mode == 'check-runtime':\n        args.append('--check-runtime')\nelif mode == 'report':\n    args.extend(['--report-run', run_path])\nelif mode == 'compare':\n    args.extend(['--compare-run', run_path])\n\nargs.extend(extra_args)\nif clean and mode in {'run', 'smoke'}:\n    cleanup()\nraise SystemExit(subprocess.call([sys.executable, *args], env={**os.environ, 'CONTAINER_RUNTIME': container_runtime}))\n\"\"\")"
