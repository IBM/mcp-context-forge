...
grpc-proto:                          ## Generate gRPC stubs for external plugin transport
	...
	@test -d "$(VENV_DIR)" || $(MAKE) venv
	@$(VENV_DIR)/bin/uv pip show grpcio-tools >/dev/null 2>&1 || $(VENV_DIR)/bin/uv pip install -q grpcio-tools
	@GOOGLE_PROTO_ROOT=$$($(VENV_DIR)/bin/python -c 'import google.api, os; p=list(google.api.__path__)[0]; print(os.path.dirname(os.path.dirname(p)))'); \
		GRPC_TOOLS_PROTO_ROOT=$$($(VENV_DIR)/bin/python -c 'import os, grpc_tools; print(os.path.join(os.path.dirname(grpc_tools.__file__), "_proto"))'); \
		$(VENV_DIR)/bin/python -m grpc_tools.protoc \
			-I mcpgateway/plugins/framework/external/grpc/proto \
			-I $$GOOGLE_PROTO_ROOT \
			-I $$GRPC_TOOLS_PROTO_ROOT \
	...
	@if [ "$$(uname)" = "Darwin" ]; then \
		...
		sed -i '' 's/^import a2a_pb2/from mcpgateway.plugins.framework.external.grpc.proto import a2a_pb2/' \
...
doctest:
	...
	@/bin/bash -c "source $(VENV_DIR)/bin/activate && \
		...
		--ignore=mcpgateway/plugins/framework/external/grpc/proto/a2a_pb2.py \
		--ignore=mcpgateway/plugins/framework/external/grpc/proto/a2a_pb2_grpc.py \
		--ignore=mcpgateway/plugins/framework/external/grpc/proto/plugin_service_pb2.py \
		--ignore=mcpgateway/plugins/framework/external/grpc/proto/plugin_service_pb2_grpc.py \
		--tb=short --no-cov --disable-warnings -n 4"
...
doctest-verbose:
	...
	@/bin/bash -c "source $(VENV_DIR)/bin/activate && \
		...
		--ignore=mcpgateway/plugins/framework/external/grpc/proto/plugin_service_pb2.py \
		--ignore=mcpgateway/plugins/framework/external/grpc/proto/plugin_service_pb2_grpc.py \
		-v --tb=short --no-cov --disable-warnings -n 4"
