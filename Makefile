...
# help: test-a2a             - Run A2A invoke tests only (real Rust queue + stub server; requires gateway_rs)
...
# A2A invoke tests (real Rust queue + stub HTTP agent). Requires gateway_rs extension.
# Runs Rust a2a_service crate tests and Python A2A tests for full coverage.
test-a2a:
	@echo "🦀 A2A invoke tests (real stack: Rust queue, invoker, stub server)..."
	@test -d "$(VENV_DIR)" || $(MAKE) venv
	@/bin/bash -c "source \"$$HOME/.cargo/env\" 2>/dev/null || true; \
		command -v cargo >/dev/null 2>&1 || { echo '❌ cargo not found; A2A tests require Rust.'; exit 1; }; \
		cargo test -p a2a_service || exit 1; \
		$(MAKE) --no-print-directory gateway-rs-install" || \
		{ echo "⚠️  Run 'make gateway-rs-install' first if A2A tests fail with ImportError."; exit 1; }
	@/bin/bash -c "export DATABASE_URL='sqlite:///:memory:' && \
		export TEST_DATABASE_URL='sqlite:///:memory:' && \
		export ARGON2ID_TIME_COST=1 && \
		export ARGON2ID_MEMORY_COST=1024 && \
		uv run pytest -n 0 -v --tb=short \
		tests/unit/mcpgateway/test_main.py tests/unit/mcpgateway/test_main_extended.py \
		-k 'a2a' && \
		uv run pytest -n 0 -v --tb=short \
		tests/unit/mcpgateway/services/test_a2a_service.py \
		tests/unit/mcpgateway/services/test_row_level_locking.py \
		tests/unit/mcpgateway/test_issue_840_a2a_agent.py \
		tests/unit/mcpgateway/services/test_metrics_buffer_service.py" || exit 1
	@echo "✅ test-a2a passed"
...
# gateway-rs-build, gateway-rs-install, gateway-rs-test are defined in the Rust section above.
