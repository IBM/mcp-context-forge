...
- **Entra inline-groups live gateway tests** ([#6931](https://github.com/IBM/mcp-context-forge/pull/6931)) - Added `tests/live_gateway/test_trust_mode_entra_inline_groups_e2e.py`, the `make testing-up-entra` target, and the `docker-compose.entra.yml` override. The suite reproduces all four inline-groups access cases against a real Entra tenant; with `AZURE_*` credentials it self-provisions every identity, including the 201-group overage user for use case 4, and deletes them after the run. Each test skips when its Entra prerequisites are absent. The override does not change the base compose file.
...
- **App-only group inquiry in the external-IdP trust funnel** - `build_trusted_external_identity` now resolves service-principal groups for app-only tokens (`idtyp == "app"`, no `groups` claim) under `JWT_TRUST_OVERAGE_POLICY=graph_lookup`, matching the bearer funnel in `get_current_user`. The lookup posts to `/servicePrincipals/{oid}/getMemberObjects` with the provider's client-credentials token; a Graph failure denies the token with reason `service_principal_groups_unresolved` (fail-closed). The default policies keep the previous behavior: app-only tokens authenticate with no team visibility. Use case 5 of the live Entra suite covers the path against a real tenant: it self-provisions a throwaway v2 application (`api.requestedAccessTokenVersion = 2` plus the `idtyp` optional claim, Microsoft Graph manifest format), places its service principal in a mapped security group, presents the app-only token, and deletes every object after the run.
