...
* **C-34 / L-13**: Permission fallback paths now rely on explicit constants and canonical permission mappings across decorators, validation, and role checks
...
* **O-08 / O-12 / O-13**: SSO/OAuth flows now enforce verified-email and trusted-domain semantics more consistently, and use opaque server-side OAuth state mapping
* **U-02 / U-03 / U-04**: Admin UI now enforces CSRF tokens/origin checks for state-changing flows, sanitizes dynamic DOM insertions, and applies SRI across external assets
