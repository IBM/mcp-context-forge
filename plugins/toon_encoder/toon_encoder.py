...
class ToonEncoderPlugin(Plugin):
    ...
    def __init__(self, config: PluginConfig) -> None:
        ...
        logger.info(f"ToonEncoderPlugin initialized: min_size={self._min_size_bytes}, " f"max_size={self._max_size_bytes}, exclude={self._exclude_tools}")
    ...
    async def tool_post_invoke(self, payload: ToolPostInvokePayload, _context: PluginContext) -> ToolPostInvokeResult:
        ...
        if modified:
            ...
            logger.info(f"ToonEncoder: Converted tool '{tool_name}' result, " f"saved {bytes_saved} bytes ({savings_pct:.1f}%), " f"took {duration_ms:.2f}ms")
    ...
    def _process_content_item(self, item: Any, tool_name: str) -> tuple[Any, bool, int, int]:
        ...
        if original_size < self._min_size_bytes:
            logger.debug(f"ToonEncoder: Skipping '{tool_name}' content " f"(size {original_size} < min {self._min_size_bytes})")
            return (item, False, 0, 0)

        if original_size > self._max_size_bytes:
            logger.debug(f"ToonEncoder: Skipping '{tool_name}' content " f"(size {original_size} > max {self._max_size_bytes})")
            return (item, False, 0, 0)
        ...
        if new_size >= original_size:
            logger.debug(f"ToonEncoder: TOON not smaller for '{tool_name}' " f"({new_size} >= {original_size}), keeping JSON")
            return (item, False, 0, 0)
