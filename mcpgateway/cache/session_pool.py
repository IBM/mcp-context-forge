...
from mcpgateway.db import engine, PoolStrategyMetric
...
class SessionPool:
    ...
    def __init__(
        ...
        logger.info(f"Initialized session pool {pool_id} for server {server_id} " f"with strategy {strategy.value}, min_size={min_size}, max_size={max_size}")
    ...
    async def acquire(self, timeout: Optional[int] = None) -> Optional[str]:
        ...
        try:
                ...
                while True:
                    async with self._lock:
                        ...
                        if session_id:
                            ...
                            await self._record_metric(session_id=session_id, response_time=wait_time, success=True, session_reused=session.reuse_count > 1, wait_time=wait_time)

                            logger.debug(f"Acquired session {session_id} from pool {self.pool_id} " f"(reuse_count={session.reuse_count}, wait_time={wait_time:.3f}s)")
                            return session_id
                        ...
                        if len(self._sessions) < self.max_size:
                            ...
                            if session:
                                ...
                                wait_time = time.time() - start_time
                                await self._record_metric(session_id=session.session_id, response_time=wait_time, success=True, session_reused=False, wait_time=wait_time)
            ...
            wait_time = time.time() - start_time
            await self._record_metric(session_id=None, response_time=wait_time, success=False, session_reused=False, wait_time=wait_time, error_message="Timeout waiting for available session")
            logger.warning(f"Timeout acquiring session from pool {self.pool_id} after {wait_time:.3f}s")
            return None
        except Exception as e:
            wait_time = time.time() - start_time
            await self._record_metric(session_id=None, response_time=wait_time, success=False, session_reused=False, wait_time=wait_time, error_message=str(e))
    ...
    async def _get_next_session(self) -> Optional[str]:
        ...
        if self.strategy == PoolStrategy.LEAST_CONNECTIONS:
            ...
            # If no best session found, fall through to default
            return self._available.popleft()

        if self.strategy == PoolStrategy.STICKY:
            # For sticky, we'd need client context - fall back to round-robin
            return self._available.popleft()

        if self.strategy == PoolStrategy.WEIGHTED:
            # For weighted, we'd need session weights - fall back to round-robin
            return self._available.popleft()

        # NONE or unknown strategy
        return self._available.popleft()
    ...
    async def _ping_session(self, session_id: str) -> bool:  # pylint: disable=unused-argument
        ...
        """
        # NOTE: Actual session health check would require transport-specific logic
        # For now, assume all sessions are healthy
        return True

    async def _record_metric(self, session_id: Optional[str], response_time: float, success: bool, session_reused: bool, wait_time: float, error_message: Optional[str] = None) -> None:  # pylint: disable=unused-argument
