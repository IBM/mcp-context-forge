...
class RedisPoolBackend:
    ...
    def __init__(self, redis_url: str, prefix: str = "mcpgateway:pool:", pool_size: int = 10, timeout: int = 5, retry_attempts: int = 3, retry_delay: float = 0.5):
        ...
        logger.info(f"Initialized Redis pool backend " f"(url={redis_url}, prefix={prefix}, pool_size={pool_size})")
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self._client = await redis.from_url(
                self.redis_url, encoding="utf-8", decode_responses=True, max_connections=self.pool_size, socket_timeout=self.timeout, socket_connect_timeout=self.timeout
            )
    ...
    async def _retry_operation(self, operation, *args, **kwargs):
        ...
        Raises:
            Exception: If all retry attempts fail, re-raises the original exception
        """
        for attempt in range(self.retry_attempts):
            try:
                ...
                if attempt < self.retry_attempts - 1:
                    ...
                    logger.warning(f"Operation failed (attempt {attempt + 1}/{self.retry_attempts}), " f"retrying in {delay}s: {e}")
    ...
    async def acquire_lock(self, pool_id: str, timeout: int = 10, blocking_timeout: Optional[int] = None) -> bool:
    ...
    async def subscribe_events(self) -> AsyncGenerator[Dict[str, Any], None]:
        ...
        Raises:
            RuntimeError: If not connected to Redis or pub/sub not initialized
            Exception: If event subscription fails
        """
        if not self._connected:
            raise RuntimeError("Not connected to Redis")
    ...
    async def set_expiring_value(self, key_name: str, value: str, ttl: int) -> None:
...
async def get_redis_backend() -> Optional[RedisPoolBackend]:
    ...
    """
    global _redis_backend  # pylint: disable=global-statement

    if _redis_backend is None:
        if settings.redis_enabled:
            _redis_backend = RedisPoolBackend(redis_url=settings.redis_url, prefix=settings.redis_pool_prefix)
