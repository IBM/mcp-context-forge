        ...
        while True:
            ...
            if timeout is not None:
                ...
                if elapsed >= timeout:
                    ...
                    logger.warning(
                        f"Rate limiter '{self.name}' timeout after {elapsed:.2f}s"
                    )
                    return False
...
class AdaptiveRateLimiter(PoolRateLimiter):
    ...
    def __init__(
        self,
        initial_max_requests: int = 1000,
        window_seconds: int = 60,
        min_requests: int = 100,
        max_requests_limit: int = 10000,
            ...
            if total >= 100:  # Adjust after 100 operations
                ...
                if success_rate > 0.95:  # >95% success rate
                    ...
                    if new_limit <= self.max_requests_limit:
                        ...
                        logger.info(
                            f"Rate limiter '{self.name}' increased limit to {new_limit} "
            ...
            if total >= 100:  # Adjust after 100 operations
                ...
                if failure_rate > 0.1:  # >10% failure rate
                    ...
                    if new_limit >= self.min_requests:
                        self.max_requests = new_limit
                        logger.warning(
                            f"Rate limiter '{self.name}' decreased limit to {new_limit} "
