...
async def get_role(role_id: str, user=Depends(get_current_user_with_permissions), db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.error(f"Failed to get role {SecurityValidator.sanitize_log_message(role_id)}: {e}")
...
async def update_role(role_id: str, role_data: RoleUpdateRequest, user=Depends(get_current_user_with_permissions), db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"Role updated: {SecurityValidator.sanitize_log_message(role_id)} by {SecurityValidator.sanitize_log_message(user['email'])}")
        db.commit()
        db.close()
        return RoleResponse.model_validate(role)
...
async def delete_role(role_id: str, user=Depends(get_current_user_with_permissions), db: Session = Depends(get_db)):
    ...
    try:
        ...
        logger.info(f"Role deleted: {SecurityValidator.sanitize_log_message(role_id)} by {SecurityValidator.sanitize_log_message(user['email'])}")
        db.commit()
        db.close()
