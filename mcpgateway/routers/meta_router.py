...
@router.post("/search_agents", response_model=SearchAgentsResponse)
async def search_agents(
    ...
    Args:
        req: Agent search request parameters
        request: FastAPI request object
        current_user_ctx: Current user context with permissions
        db: Database session

    Returns:
        SearchAgentsResponse: Ranked agent search results
    ...
    try:
        meta_service = get_meta_server_service()
        arguments = req.model_dump()
        result = await meta_service._search_agents(arguments)
        return SearchAgentsResponse(**result)
    except Exception as e:
        logger.error(f"Error searching agents: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error")


@router.post("/delegate_task", response_model=DelegateTaskResponse)
async def delegate_task(
    ...
    Args:
        req: Delegation request parameters
        request: FastAPI request object
        current_user_ctx: Current user context with permissions
        db: Database session

    Returns:
        DelegateTaskResponse: Delegation result
    ...
    try:
        meta_service = get_meta_server_service()
        arguments = req.model_dump()
        result = await meta_service._delegate_task(arguments)
        return DelegateTaskResponse(**result)
    except Exception as e:
        logger.error(f"Error delegating task: {e}")
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Internal server error")


@router.post("/orchestrate_agents", response_model=OrchestrateResponse)
async def orchestrate_agents(
    ...
    Args:
        req: Orchestration request parameters
        request: FastAPI request object
        current_user_ctx: Current user context with permissions
        db: Database session

    Returns:
        OrchestrateResponse: Orchestration result
    ...
    try:
        meta_service = get_meta_server_service()
        arguments = req.model_dump()
        result = await meta_service._orchestrate_agents(arguments)
