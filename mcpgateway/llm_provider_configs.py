...
PROVIDER_CONFIGS: Dict[str, ProviderConfigDefinition] = {
    ...
    "portkey": ProviderConfigDefinition(
        provider_type="portkey",
        display_name="Portkey",
        description="Portkey AI Gateway (hosted or self-hosted)",
        requires_api_key=False,
        api_key_label="Upstream Provider API Key",
        api_key_help="Optional when using a Portkey-managed provider slug or virtual key; otherwise this is the upstream provider token forwarded as Authorization.",
        requires_api_base=True,
        api_base_default="http://localhost:8787/v1",
        api_base_help="Base URL of your Portkey gateway, for example http://localhost:8787/v1 or http://portkey:8787/v1 in docker compose",
        config_fields=[
            ProviderFieldDefinition(
                name="provider",
                label="Portkey Provider",
                field_type="text",
                required=True,
                placeholder="openai or @my-provider",
                help_text="Portkey provider slug such as openai, anthropic, azure-openai, bedrock, vertex-ai, groq, together-ai, or a managed provider slug like @my-provider.",
            ),
            ProviderFieldDefinition(
                ...
                required=False,
                placeholder="pk-...",
                help_text="Optional for self-hosted sidecars; required for hosted or protected Portkey deployments.",
            ),
            ProviderFieldDefinition(
                name="portkey_config",
                ...
                field_type="textarea",
                required=False,
                placeholder='pc-xxxxx or {"retry":{"attempts":3}}',
                help_text="Portkey config ID or inline JSON sent as x-portkey-config.",
            ),
            ProviderFieldDefinition(
                name="virtual_key",
                label="Virtual Key",
            ...
            ProviderFieldDefinition(
                ...
                field_type="text",
                required=False,
                placeholder="https://my-private-openai-compatible.example.com/v1",
                help_text="Optional x-portkey-custom-host for private or OpenAI-compatible upstreams routed through Portkey.",
            ),
            ProviderFieldDefinition(
                ...
                field_type="textarea",
                required=False,
                placeholder='["x-trace-id","x-request-id"]',
                help_text="Optional JSON array or comma-separated list for x-portkey-forward-headers.",
            ),
        ],
    ),
