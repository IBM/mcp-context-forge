...
def get_security_warnings(settings: Settings) -> list[str]:
    ...
    if len(pw) < 12:
        warnings.append(f"Admin password should be at least 12 characters long. Current length: {len(pw)}")
    ...
    if len(basic_pw) < 12:
        warnings.append(f"BASIC_AUTH_PASSWORD should be at least 12 characters long. Current length: {len(basic_pw)}")
