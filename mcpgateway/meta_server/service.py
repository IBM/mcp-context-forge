...
class MetaServerService:
    ...
    def get_meta_tool_definitions(self) -> List[Dict[str, Any]]:
        ...
        Examples:
            >>> service = MetaServerService()
            >>> tools = service.get_meta_tool_definitions()
            >>> {t["name"] for t in tools} == {"search_tools", "list_tools", "describe_tool", "execute_tool", "get_tool_categories", "get_similar_tools", "search_agents", "delegate_task", "orchestrate_agents"}
            True
        """
    ...
    async def _search_tools(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # -- Step 4.5: Apply collaborative filtering boost (if enabled and user context available) --
        # Augment semantic/keyword scores with collaborative filtering signals based on
        # similar users' tool usage patterns.
        user_email = arguments.get("user_email")
        if user_email and settings.collaborative_filtering_enabled and settings.cf_boost_weight > 0.0:
            try:
                from mcpgateway.services.collaborative_recommender import collaborative_recommender

                # Get boost scores for all candidate tools
                candidate_tool_ids = [r.tool_name for r in ranked_results]
                boost_scores = await collaborative_recommender.get_boost_scores(user_email, candidate_tool_ids)

                # Apply weighted boost to similarity scores
                for result in ranked_results:
                    if result.tool_name in boost_scores:
                        cf_boost = boost_scores[result.tool_name]
                        # Combine: weighted average of semantic score and CF boost
                        # semantic_weight = (1 - cf_boost_weight), cf_weight = cf_boost_weight
                        original_score = result.similarity_score
                        result.similarity_score = (
                            original_score * (1 - settings.cf_boost_weight) +
                            cf_boost * settings.cf_boost_weight
                        )
                        logger.debug(
                            f"CF boost for {result.tool_name}: {original_score:.3f} → {result.similarity_score:.3f} "
                            f"(CF={cf_boost:.3f}, weight={settings.cf_boost_weight})"
                        )

                # Re-sort after boosting
                ranked_results = sorted(ranked_results, key=lambda r: r.similarity_score, reverse=True)
                logger.debug(f"Applied collaborative filtering boost for {user_email} to {len(boost_scores)} tools")
            except Exception as e:
                logger.warning(f"Collaborative filtering boost failed: {e}", exc_info=True)
                # Continue with original scores on failure
    ...
    async def _search_agents(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Search for A2A agents using semantic similarity.

        Args:
            arguments: Search parameters with keys: query, limit, tags, agent_type, search_field.

        Returns:
            SearchAgentsResponse as dict.
        """
        query = arguments.get("query", "")
        limit = arguments.get("limit", 10)
        tags = arguments.get("tags", [])
        agent_type = arguments.get("agent_type")
        search_field = arguments.get("search_field", "description")

        if not query:
            return SearchAgentsResponse(
                agents=[], total_count=0, query=query,
            ).model_dump(by_alias=True)

        try:
            from mcpgateway.services.agent_semantic_search_service import get_agent_semantic_search_service
    ...
    async def _delegate_task(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        Args:
            arguments: Delegation parameters with keys: agent_name, task, parameters,
                       timeout, retry_count, correlation_id.

        Returns:
            DelegateTaskResponse as dict.
        """
        import asyncio
        import time
        ...
        timeout = arguments.get("timeout") or settings.a2a_delegation_timeout
        retry_count = arguments.get("retry_count")
        if retry_count is None:
            retry_count = settings.a2a_delegation_retry_attempts
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                db = next(db_gen)
                try:
                    invoke_params = {**parameters, "task": task}
                ...
                last_error = f"Agent '{agent_name}' timed out after {timeout}s (attempt {attempt + 1}/{retry_count + 1})"
                logger.warning(last_error)
            except Exception as e:
                last_error = f"Agent '{agent_name}' invocation failed: {e} (attempt {attempt + 1}/{retry_count + 1})"
                logger.warning(last_error)
    ...
    async def _orchestrate_agents(self, arguments: Dict[str, Any]) -> Dict[str, Any]:
        """Execute a multi-agent workflow.

        Args:
            arguments: Orchestration parameters with keys: steps, execution_mode, correlation_id.

        Returns:
            OrchestrateResponse as dict.
        """
        import time
        ...
        if not settings.a2a_orchestration_enabled:
            return OrchestrateResponse(
                correlation_id=correlation_id,
                success=False,
                steps=[],
                total_execution_time_ms=0,
                aggregated_result={"error": "Orchestration is disabled"},
            ).model_dump(by_alias=True)
        ...
        if not steps:
            return OrchestrateResponse(
                ...
                steps=[],
                total_execution_time_ms=0,
                aggregated_result={"error": "No steps provided"},
            ).model_dump(by_alias=True)

        try:
            from mcpgateway.services.agent_orchestration_service import AgentOrchestrationService

            orch_service = AgentOrchestrationService()
            start_time = time.time()
            result = await orch_service.execute(
                steps=steps,
