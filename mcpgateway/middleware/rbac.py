        ...
        if settings.trust_proxy_auth:
            ...
            if proxy_user:
                ...
                if proxy_user == settings.platform_admin_email:
                    ...
                    # Use fresh_db_session() for short-lived lookup to avoid holding connections
                    try:
                        # Third-Party
                        ...
                        with fresh_db_session() as lookup_db:
                            user = lookup_db.execute(select(EmailUser).where(EmailUser.email == proxy_user)).scalar_one_or_none()
                            if user:
...
def require_permission(permission: str, resource_type: Optional[str] = None):
    ...
    Examples:
        ...
        >>> with patch('mcpgateway.middleware.rbac.PermissionService', DummyPS), patch('mcpgateway.middleware.rbac.fresh_db_session'):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        async def wrapper(*args, **kwargs):
            ...
            """
            # Extract user context from known parameter names only (Issue #2319)
            # Security: Only check specific parameter names to prevent request body
            # injection attacks where a malicious payload could be mistaken for user context
            user_context = None
            for param_name in ("current_user_ctx", "user_ctx", "user", "current_user", "_user"):
