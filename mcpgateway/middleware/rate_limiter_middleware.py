...
# Limit
from limits import parse, RateLimitItem, WindowStats
...
class RateLimiterMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, metric_service: ProtectionMetricsService):
        ...
        Args:
            ...
            metric_service: The protection metrics service
        """
        super().__init__(app)
    ...
    async def dispatch(self, request: Request, call_next: Callable[[Request], Awaitable[Response]]) -> Response:
        ...
        if not settings.experimental_protection_suite or not settings.rate_limiting_enabled or self.limiter is None or self._is_request_whitelisted(request) or self._is_admin_bypass_request(request):
        ...
        try:
            ...
            if 0 < remaining <= (limit.amount * (1 - warning_threshold)):
        ...
        if not is_allowed:
            ...
            response = JSONResponse(status_code=429, content={"detail": f"Rate limit exceeded for {request.url.path}. Please try again later."})
    ...
    async def send_alert(self, limit_key: str, request: Request, is_blocked: bool, identifier: str, limit: RateLimitItem, reset_time: int, window_stats: Optional[WindowStats]):
        """
        ...
        Args:
            ...
            limit: The rate limit item
            reset_time: The reset time
            window_stats: The window statistics
        """
    ...
    def _is_request_whitelisted(self, request: Request) -> bool:
        ...
        """
        return self._is_ip_whitelisted(request) or self._is_user_agent_whitelisted(request) or self._is_api_key_whitelisted(request)
