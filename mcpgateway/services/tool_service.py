...
class ToolService:
    ...
    def _convert_tool_to_read(self, tool: DbTool, include_metrics: bool = True) -> ToolRead:
        ...
        # Guarantee required base fields exist even if SQLAlchemy hasn't materialized them in __dict__ yet
        tool_dict["id"] = getattr(tool, "id", tool_dict.get("id"))
        tool_dict["original_name"] = getattr(tool, "original_name", tool_dict.get("original_name"))
        tool_dict["url"] = getattr(tool, "url", tool_dict.get("url"))
        tool_dict["description"] = getattr(tool, "description", tool_dict.get("description"))
        tool_dict["integration_type"] = getattr(tool, "integration_type", tool_dict.get("integration_type"))
        tool_dict["headers"] = getattr(tool, "headers", tool_dict.get("headers"))
        tool_dict["input_schema"] = getattr(tool, "input_schema", tool_dict.get("input_schema"))
        tool_dict["jsonpath_filter"] = getattr(tool, "jsonpath_filter", tool_dict.get("jsonpath_filter"))
        tool_dict["created_at"] = getattr(tool, "created_at", tool_dict.get("created_at"))
        tool_dict["updated_at"] = getattr(tool, "updated_at", tool_dict.get("updated_at"))
        tool_dict["enabled"] = getattr(tool, "enabled", tool_dict.get("enabled"))
        tool_dict["reachable"] = getattr(tool, "reachable", tool_dict.get("reachable"))
        tool_dict["gateway_id"] = getattr(tool, "gateway_id", tool_dict.get("gateway_id"))
