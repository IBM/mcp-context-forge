...
from mcpgateway.db import AsyncSessionLocal, fresh_async_db_session, fresh_db_session, StructuredLogEntry
...
class LogRouter:
    ...
    def route(self, entry: Dict[str, Any], db: Optional[Session] = None) -> None:
        """Route log entry to configured destinations (sync version).
    ...
    async def route_async(self, entry: Dict[str, Any], db: Optional[AsyncSession] = None) -> None:
        """Route log entry to configured destinations (async version).
        ...
        # Persist to database if enabled
        if self.database_enabled:
            await self._persist_to_database_async(entry, db)
    ...
    def _persist_to_database(self, entry: Dict[str, Any], db: Optional[Session] = None) -> None:
        """Persist log entry to database (sync version).
        ...
        """
        if db is None:
            # Use fresh_db_session context manager for proper session management
            with fresh_db_session() as fresh_db:
                self._do_persist_to_database(entry, fresh_db)
            return

        self._do_persist_to_database(entry, db)

    async def _persist_to_database_async(self, entry: Dict[str, Any], db: Optional[AsyncSession] = None) -> None:
        """Persist log entry to database (async version).
        ...
        if AsyncSessionLocal is None:
            ...
            self._persist_to_database(entry, None)
            return

        if db is None:
            # Use fresh_async_db_session context manager for proper session management
            async with fresh_async_db_session() as fresh_db:
                await self._do_persist_to_database_async(entry, fresh_db)
            return

        await self._do_persist_to_database_async(entry, db)

    def _build_log_entry(self, entry: Dict[str, Any]) -> StructuredLogEntry:
        """Build a StructuredLogEntry from the entry dictionary.

        Args:
            entry: Log entry dictionary

        Returns:
            StructuredLogEntry instance
        """
    ...
    def _do_persist_to_database(self, entry: Dict[str, Any], db: Session) -> None:
        """Internal method to persist log entry to database (sync).
        ...
        """
        try:
            log_entry = self._build_log_entry(entry)
            db.add(log_entry)
            db.commit()
    ...
    async def _do_persist_to_database_async(self, entry: Dict[str, Any], db: AsyncSession) -> None:
        """Internal method to persist log entry to database (async).
...
class StructuredLogger:
    ...
    async def log_async(
        ...
        **kwargs: Any,
        ...
        """Log a structured message (async version).
        ...
        # Route to destinations
        await self.router.route_async(entry, db)
