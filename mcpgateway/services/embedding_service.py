...
class EmbeddingService:
    ...
    def __init__(self):
        ...
        self._embedding_dim: int = 0  # set after probe in initialize()
        # Store provider settings — api_key stays as SecretStr until initialize()
    ...
    async def initialize(self):
        ...
        Guarded by an asyncio.Lock so concurrent coroutines cannot race and
        trigger multiple initializations under load.  The API key secret value
        is extracted here — at the last possible moment — rather than in
        __init__, so it is never present in plain-text form before it is
        actually needed.
        """
        async with self._lock:
            ...
            self._provider = EmbeddingProviderFactory.create(config)
            self._embedding_model = self._provider.get_embedding_model()

            # Probe the model to determine its true output dimension.
            # This catches model/config mismatches at startup rather than
            # silently corrupting the vector store on first insert.
            probe: list = await self._embedding_model.aembed_query("probe")
            detected_dim = len(probe)
            expected_dim = settings.embedding_dim

            if detected_dim != expected_dim:
                raise ValueError(
                    f"Embedding dimension mismatch: model '{self._model_name}' produces "
                    f"{detected_dim}-dimensional vectors, but EMBEDDING_DIM is configured as "
                    f"{expected_dim}. Update EMBEDDING_DIM in your environment to match the "
                    f"model (e.g. text-embedding-3-small=1536, text-embedding-3-large=3072)."
                )
    ...
    def get_provider_info(self) -> Dict[str, Any]:
        ...
        return {"provider": self._provider_name, "model": self._model_name, "initialized": self._initialized}

    def store_tool_embedding(self, db: Session, tool_id: str, embedding: List[float], model_name: str) -> ToolEmbedding:
        ...
        if self._embedding_dim > 0 and len(embedding) != self._embedding_dim:
            raise ValueError(
                f"Embedding dimension mismatch: got {len(embedding)}-dimensional vector "
                f"but service is initialized with {self._embedding_dim} dimensions. "
                f"Ensure all embeddings are generated by the same model."
            )
        ...
        try:
            ...
            existing = db.query(ToolEmbedding).filter(ToolEmbedding.tool_id == tool_id).first()
    ...
    def batch_store_tool_embeddings(self, db: Session, tool_embeddings: List[tuple[str, List[float]]], model_name: str) -> List[ToolEmbedding]:
        ...
        try:
            ...
            existing_map = {emb.tool_id: emb for emb in db.query(ToolEmbedding).filter(ToolEmbedding.tool_id.in_(tool_ids)).all()}
    ...
    async def embed_and_store_tool(self, db: Session, tool) -> ToolEmbedding:
    ...
    async def embed_and_store_tools_batch(self, db: Session, tools: List) -> List[ToolEmbedding]:
