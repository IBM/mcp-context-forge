...
function createEnhancedTopPerformersSection(topData) {
    try {
        ...
        exportButton.className = 'mt-4 bg-indigo-600 text-white px-4 py-2 rounded hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600';
...
function createTopPerformersTable(entityType, data, isActive) {
    ...
    headers.forEach((headerText, index) => {
        ...
        if (index === 0) th.setAttribute('aria-sort', 'ascending');
    ...
    paginatedData.forEach((item, index) => {
        ...
        rankCell.className = 'px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-gray-100 sm:px-6 sm:py-4';
        ...
        nameCell.className = 'px-6 py-4 whitespace-nowrap text-sm text-indigo-600 dark:text-indigo-400 cursor-pointer';
        ...
        nameCell.setAttribute('aria-label', `View details for ${item.name || 'Unknown'}`);
        row.appendChild(nameCell);
        
        // Executions
        const execCell = document.createElement('td');
        execCell.className = 'px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 sm:px-6 sm:py-4';
        ...
        avgTimeCell.className = 'px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 sm:px-6 sm:py-4';
        ...
        successCell.className = 'px-6 py-4 whitespace-nowrap text-sm sm:px-6 sm:py-4';
        ...
        successBadge.className = `inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
            successRate >= 95 ? 'bg-green-100 text-green-800 dark:bg-green-800 dark:text-green-100' :
            successRate >= 80 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-800 dark:text-yellow-100' :
        ...
        lastUsedCell.className = 'px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-300 sm:px-6 sm:py-4';
...
function exportMetricsToCSV(topData) {
    const headers = ['Entity Type', 'Rank', 'Name', 'Executions', 'Avg Response Time', 'Success Rate', 'Last Used'];
    const rows = [];
        ...
        if (topData[type] && Array.isArray(topData[type])) {
            topData[type].forEach((item, index) => {
                rows.push([
                    ...
                    item.avg_response_time || item.avgResponseTime ? `${Math.round(item.avg_response_time || item.avgResponseTime)}ms` : 'N/A',
