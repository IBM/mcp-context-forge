...
# First-Party
from mcpgateway.utils.pgvector import HAS_PGVECTOR
...
def upgrade() -> None:
    ...
    if dialect_name == "postgresql":
        ...
        if not HAS_PGVECTOR:
            raise ImportError("pgvector is required for PostgreSQL. " "Install with: pip install pgvector")
    ...
    op.create_table(
        ...
        embedding_col,
        sa.Column("model_name", sa.String(255), nullable=False, server_default="text-embedding-3-small"),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.text("now()" if dialect_name == "postgresql" else "(datetime('now'))"), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.text("now()" if dialect_name == "postgresql" else "(datetime('now'))"), nullable=False),
        sa.PrimaryKeyConstraint("id", name="pk_tool_embeddings"),
        sa.ForeignKeyConstraint(["tool_id"], ["tools.id"], name="fk_tool_embeddings_tool_id", ondelete="CASCADE"),
    )
