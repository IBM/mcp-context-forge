        ...
        try:
            ...
            for uq in table.constraints:
                if isinstance(uq, sa.UniqueConstraint):
                    uq_colnames = set([col.name for col in uq.columns])
                    # Skip if this is a unique constraint only on 'name'
                    if uq_colnames == {'name'}:
                        continue
                    # Also skip if this is the constraint being replaced by the new one
                    if any(uq.name == c[1] if uq.name else False for c in constraints):
                        continue
