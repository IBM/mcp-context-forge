    ...
    for tbl, constraints in {
        ...
        try:
            ...
            uqs_to_copy = [
                ...
                if isinstance(uq, sa.UniqueConstraint) and table=='prompts'  # Only apply to prompts table
                and set([col.name for col in uq.columns]) != {'name'}
                and not any(uq.name == c[1] if uq.name else False for c in constraints)
            ]
            print(f"Unique constraints to copy for {tbl}: {[uq.name for uq in uqs_to_copy]}")
