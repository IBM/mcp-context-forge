...
async def _resolve_teams_from_db(email: str, user_info) -> Optional[List[str]]:
    ...
    Args:
        email: Identity for the lookup (e-mail or canonical user_id). The
            membership and cache lookups canonicalize it through
            ``_resolve_canonical_user_id_sync`` so e-mail identities from
            legacy or UUID-sub tokens reach user_id-keyed rows.
