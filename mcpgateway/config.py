...
class Settings(BaseSettings):
    ...
    ldap_enabled: bool = Field(default=False, description="Enable LDAP/Active Directory authentication")
    ldap_uri: str = Field(default="ldap://localhost:389", description="LDAP server URI (ldap:// or ldaps://)")
    ldap_base_dn: str = Field(default="dc=example,dc=org", description="LDAP base DN for all searches")
    ldap_bind_dn: str = Field(default="cn=admin,dc=example,dc=org", description="DN of the service account for LDAP searches")
    ldap_bind_password: SecretStr = Field(default=SecretStr("admin"), description="Password for the LDAP bind DN")
    ldap_user_search_base: str = Field(default="ou=Users", description="Relative DN under base DN for user searches")
    ldap_user_search_filter: str = Field(default="(uid={username})", description="LDAP filter for user lookup ({username} is replaced)")
    ldap_user_email_attribute: str = Field(default="mail", description="LDAP attribute containing the user's email")
    ldap_user_name_attribute: str = Field(default="cn", description="LDAP attribute containing the user's display name")
    ldap_user_uid_attribute: str = Field(default="uid", description="LDAP attribute containing the user's UID/login name")
    ldap_group_search_base: str = Field(default="ou=Groups", description="Relative DN under base DN for group searches")
    ldap_group_search_filter: str = Field(default="(objectClass=groupOfNames)", description="LDAP filter for group lookup")
    ldap_group_member_attribute: str = Field(default="member", description="LDAP attribute listing group members (DN or uid)")
    ldap_group_name_attribute: str = Field(default="cn", description="LDAP attribute containing the group name")
    ldap_use_ssl: bool = Field(default=False, description="Use LDAPS (SSL/TLS) for LDAP connections")
    ldap_start_tls: bool = Field(default=False, description="Use StartTLS for LDAP connections")
    ldap_tls_validate: bool = Field(default=True, description="Validate TLS certificates (disable for self-signed certs in dev)")
    ldap_connect_timeout: int = Field(default=5, description="LDAP connection timeout in seconds")
    ldap_search_timeout: int = Field(default=10, description="LDAP search timeout in seconds")
    ldap_sync_enabled: bool = Field(default=False, description="Enable periodic LDAP directory sync")
    ldap_sync_interval_minutes: int = Field(default=60, description="LDAP sync interval in minutes")
    ldap_sync_delete_orphans: bool = Field(default=False, description="Delete users/groups no longer in LDAP during sync")
    ldap_auto_create_users: bool = Field(default=True, description="Automatically create gateway users from LDAP on login")
    ldap_auto_create_teams: bool = Field(default=True, description="Automatically create gateway teams from LDAP groups")
    ldap_role_mappings: Dict[str, str] = Field(default_factory=dict, description="Map LDAP groups to gateway roles (JSON: {ldap_group: role_name})")
    ldap_default_role: Optional[str] = Field(default="viewer", description="Default role for LDAP users without group mapping")
    ldap_page_size: int = Field(default=500, description="Page size for LDAP paged searches")
