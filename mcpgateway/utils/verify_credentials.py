...
async def require_auth(request: Request, credentials: Optional[HTTPAuthorizationCredentials] = Depends(security), jwt_token: Optional[str] = Cookie(default=None)) -> str | dict:
    ...
    if not token and jwt_token:
        ...
        logger.info(
            ...
            "token_prefix=%s",
            "cookie"
            if (hasattr(request, "cookies") and request.cookies.get("jwt_token"))
            else ("header" if (credentials and credentials.credentials) else ("fastapi_cookie" if jwt_token else "none")),
