...
def build_gateway_auth_headers(gateway: DbGateway) -> Dict[str, str]:
    ...
    return {k: SecurityValidator.sanitize_credential_value(v) for k, v in _bearer_basic_headers(auth_type, decoded).items()}
...
def normalize_downstream_auth_headers(auth_type: Optional[str], auth_value: Optional[Any]) -> Dict[str, str]:
    ...
    # this validation existed, so a gateway configured before this change self-heals
    # without requiring a manual re-save.
    strict = {k: SecurityValidator.sanitize_credential_value(v) for k, v in strict.items()}
