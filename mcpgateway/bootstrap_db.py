        ...
        with cast(Any, SessionLocal)() as db:
            ...
            default_roles = [
                {"name": "platform_admin", "description": "Platform administrator with all permissions", "scope": "global", "permissions": ["*"], "is_system_role": True},  # All permissions
                {
                    ...
                    "scope": "team",
                    "permissions": ["teams.create", "teams.read", "teams.update", "teams.join", "teams.manage_members", "gateways.create", "tools.read", "tools.execute", "resources.read", "prompts.read"],
                    "is_system_role": True,
                },
                {
                    "name": "team_admin",
            ...
            if platform_admin_role:
                try:
                    ...
                    if not existing_assignment or not existing_assignment.is_active:
                        ...
                        if team:
                            team_owner_role = await role_service.get_role_by_name("team_owner", "team")
                            if team_owner_role:
