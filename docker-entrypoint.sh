...
PRAXIS_PROXY_ENABLED="${PRAXIS_PROXY_ENABLED:-false}"
PRAXIS_PROXY_CONFIG="${PRAXIS_PROXY_CONFIG:-/app/praxis.yaml}"
PRAXIS_PROXY_LISTEN="${PRAXIS_PROXY_LISTEN:-127.0.0.1:8789}"
PRAXIS_PROXY_LOG="${PRAXIS_PROXY_LOG:-info}"
...
cleanup() {
    ...
    if [[ -n "${PRAXIS_PROXY_PID}" ]] && kill -0 "${PRAXIS_PROXY_PID}" 2>/dev/null; then
        pids+=("${PRAXIS_PROXY_PID}")
    fi
...
print_a2a_runtime_mode() {
    ...
    if [[ "${EXPERIMENTAL_RUST_A2A_RUNTIME_ENABLED}" = "true" ]]; then
        echo "WARNING: The Rust A2A runtime sidecar is deprecated as of 2026-06-11 and will sunset on 2026-07-07. Use the default Python A2A invocation path. See https://ibm.github.io/mcp-context-forge/deprecations/."
...
start_managed_praxis_proxy() {
    local proxy_bin="/app/bin/praxis-proxy"
    local proxy_config="${PRAXIS_PROXY_CONFIG:-/app/praxis.yaml}"
    local proxy_listen="${PRAXIS_PROXY_LISTEN:-127.0.0.1:8789}"

    if [[ ! -x "${proxy_bin}" ]]; then
        echo "WARNING: PRAXIS_PROXY_ENABLED=true but Praxis proxy binary not found at ${proxy_bin}"
        echo "Praxis proxy will not start. Set PRAXIS_PROXY_ENABLED=false to suppress this warning."
        return 0
    fi

    if [[ ! -f "${proxy_config}" ]]; then
        echo "WARNING: Praxis proxy config not found at ${proxy_config}"
        echo "Praxis proxy will not start. Provide a valid config via PRAXIS_PROXY_CONFIG."
        return 0
    fi

    export PRAXIS_PROXY_CONFIG="${proxy_config}"
    export PRAXIS_PROXY_LISTEN="${proxy_listen}"
    if [[ -n "${PRAXIS_PROXY_LOG:-}" ]]; then
        export RUST_LOG="${PRAXIS_PROXY_LOG}"
    fi

    echo "Starting Praxis proxy on ${PRAXIS_PROXY_LISTEN} (config: ${PRAXIS_PROXY_CONFIG})..."
    "${proxy_bin}" --config "${proxy_config}" --listen "${proxy_listen}" &
    PRAXIS_PROXY_PID=$!

    # Simple startup delay - Praxis doesn't expose a health endpoint yet
    sleep 2
    if ! kill -0 "${PRAXIS_PROXY_PID}" 2>/dev/null; then
        echo "ERROR: Praxis proxy failed to start"
        exit 1
    fi
    echo "Praxis proxy started successfully (PID: ${PRAXIS_PROXY_PID})"
}
...
if [[ ( "${EXPERIMENTAL_RUST_MCP_RUNTIME_ENABLED}" = "true" && "${EXPERIMENTAL_RUST_MCP_RUNTIME_MANAGED}" = "true" ) || ( "${EXPERIMENTAL_RUST_A2A_RUNTIME_ENABLED}" = "true" && "${EXPERIMENTAL_RUST_A2A_RUNTIME_MANAGED}" = "true" ) || "${PRAXIS_PROXY_ENABLED}" = "true" ]]; then
    ...
    if [[ "${PRAXIS_PROXY_ENABLED}" = "true" ]]; then
        start_managed_praxis_proxy
    fi
    "${SERVER_CMD[@]}" &
    SERVER_PID=$!
    ...
    if [[ -n "${PRAXIS_PROXY_PID}" ]]; then
        WAIT_PIDS+=("${PRAXIS_PROXY_PID}")
    fi
