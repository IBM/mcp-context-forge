...
describe("OAuth2Auth", () => {
  ...
  it("should only show automatic credential guidance for authorization_code", () => {
    const guidance =
      "Optional. Some authorization servers issue these automatically when you authorize.";
    const { rerender } = render(<OAuth2Auth {...defaultProps} />);

    expect(screen.queryByText(guidance)).not.toBeInTheDocument();

    rerender(<OAuth2Auth {...defaultProps} grantType="authorization_code" />);
