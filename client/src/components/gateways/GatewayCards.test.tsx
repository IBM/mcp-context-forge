...
describe("VirtualServerCard", () => {
  ...
  it("shows dropdown menu with view details option", async () => {
    ...
    // Find the actions dropdown trigger (ghost icon button)
    const actionButtons = screen.getAllByRole("button");
    const ellipsisBtn = actionButtons.find((b) =>
      b.getAttribute("aria-label")?.includes("Actions for")
    );
    expect(ellipsisBtn).toBeTruthy();
    fireEvent.click(ellipsisBtn!);
    // After click, view details menu item appears
    const viewDetailsItem = await screen.findByText(/gateways\.card\.viewDetails/i);
    expect(viewDetailsItem).toBeTruthy();
  });

  it("shows Edit option in dropdown when onEdit provided", async () => {
    const onEdit = vi.fn();
    renderWithProviders(
    ...
    fireEvent.click(ellipsisBtn!);
    const editItem = await screen.findByText(/gateways\.card\.editServer/i);
  ...
  it("shows Delete option when onDelete provided", async () => {
    ...
    fireEvent.click(ellipsisBtn!);
    const deleteItem = await screen.findByText(/common\.button\.delete/i);
  ...
  it("shows Activate/Deactivate toggle when onToggleStatus provided", async () => {
    ...
    const toggle = await screen.findByText("Deactivate");
