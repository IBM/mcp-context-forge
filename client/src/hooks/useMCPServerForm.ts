...
const mcpServerFormSchema = z.object({
  ...
  auth_headers: z
    ...
    .optional(),
  auth_query_param_key: z.string().transform((val) => sanitizeQueryParam(val, 100)).optional(),
  auth_query_param_value: z.string().transform((val) => sanitizeQueryParam(val, 500)).optional(),
...
export function useMCPServerForm(gatewayId?: string): UseMCPServerFormReturn {
  ...
  useEffect(() => {
    if (serverData && gatewayId) {
      ...
      if (serverData.authHeaders && serverData.authHeaders.length > 0) {
        setCustomHeaders(serverData.authHeaders.map((h, i) => ({ id: String(i + 1), key: h.key, value: h.value })));
      } else if (serverData.authHeaderKey) {
        setCustomHeaders([{ id: "1", key: serverData.authHeaderKey, value: serverData.authHeaderValue || "" }]);
      }
      // query param auth
      if (serverData.authQueryParamKey) setQueryParamName(serverData.authQueryParamKey);
      if (serverData.authQueryParamValueMasked) setQueryParamApiKey(serverData.authQueryParamValueMasked);
      ...
      if (serverData.oauthConfig) {
        ...
        if (oauthConfig.scopes) {
          setOAuthScopes(Array.isArray(oauthConfig.scopes) ? oauthConfig.scopes.join(" ") : oauthConfig.scopes);
        }
  ...
  const getFormData = useCallback((): MCPServerFormData => {
    ...
    return {
      ...
      authPassword: authType === "basic" ? (authPassword === "*****" ? undefined : authPassword || undefined) : undefined,
      authToken: authType === "bearer" ? (bearerToken === "*****" ? undefined : bearerToken || undefined) : undefined,
      auth_headers: (() => {
